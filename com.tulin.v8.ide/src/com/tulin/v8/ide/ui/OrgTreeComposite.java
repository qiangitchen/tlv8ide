...
public class OrgTreeComposite extends Composite {
	...
	public OrgTreeComposite(Composite parent, int style) {
		...
		searchbtn.setImage(StudioPlugin.getIcon("search.gif"));
	...
	private void doTreeSearch() {
		List<Map> slist = OrgUtils.getOrgViewSearchList(searchtext.getText(),
				filter);
		if (slist.size() > 0) {
			...
			MessageDialog.openInformation(getShell(), "提示", "没有找到["
					+ searchtext.getText() + "]相关内容!");
		}
	}
	...
	public void initTreeData() {
		Orgview.removeAll();
		orgItem.clear();
		List<Map> roots = OrgUtils.getOrgTreeRootData(rootFilter, filter,
				orderby);
		for (int i = 0; i < roots.size(); i++) {
			...
			item.setText(name);
			item.setImage(StudioPlugin.getIcon(createIcon(kind)));
			item.setData("id", id);
			item.setData("map", m);
			orgItem.put(id, item);
	...
	private void chilTreeData(TreeItem pitem, String parent) {
		List<Map> datas = OrgUtils.getOrgTreeChiledData(parent, rootFilter,
				filter, orderby);
	...
	public void addTreeSelectionListener(
			OrgTreeSelectionAdapter orgtreeselectionadapter) {
