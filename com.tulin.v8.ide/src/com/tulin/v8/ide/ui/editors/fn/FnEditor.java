...
import com.tulin.v8.ide.ui.editors.MutiPageContentOutlinePage;
import com.tulin.v8.ide.ui.editors.Z;
...
public class FnEditor extends FormEditor implements IResourceChangeListener{
	public static String ID = "com.tulin.v8.ide.ui.editors.fn.FnEditor";
	...
	void createPage0() {
		try {
			...
			ErrorDialog.openError(getSite().getShell(),
					Messages.getString("editors.FnEditor.2"), null,
					e.getStatus());
		}
	}
	...
	class MasterDetailsPage extends FormPage {
		...
		public MasterDetailsPage(FormEditor editor) {
			super(editor, "masterDetail", Messages
					.getString("editors.FnEditor.3"));
		}
		...
		protected void createFormContent(final IManagedForm managedForm) {
			...
			ToolBar toolbar = new ToolBar(composite, SWT.FLAT | SWT.WRAP
					| SWT.RIGHT);
			toolbar.setBackgroundImage(StudioPlugin
					.getIcon("editors/top_banner.gif"));
			...
			addFn.setImage(StudioPlugin.getIcon("addbtn.gif"));
			addFn.setText(Messages.getString("editors.FnEditor.6"));
			removeFn = new ToolItem(toolbar, SWT.PUSH);
			removeFn.setImage(StudioPlugin.getIcon("delbtn.gif"));
			...
			Section section1 = toolkit.createSection(sashForm,
					Section.TITLE_BAR);
			section1.setText(Messages.getString("editors.FnEditor.8"));
			tree = new Tree(section1, SWT.V_SCROLL | GridData.FILL_VERTICAL
					| SWT.BORDER);
			...
			codetext = new Text(client, SWT.BORDER | SWT.MULTI | SWT.WRAP
					| SWT.LEFT);
			...
			helptext = new Text(client, SWT.BORDER | SWT.MULTI | SWT.WRAP
			...
			Section section3 = toolkit.createSection(child, Section.DESCRIPTION
					| Section.TITLE_BAR);
			...
			ToolBar patoolbar = new ToolBar(paramgp, SWT.FLAT | SWT.WRAP
					| SWT.RIGHT);
			...
			removeparam.setImage(StudioPlugin.getIcon("delete.gif"));
			removeparam.setText(Messages.getString("editors.FnEditor.16"));
			removeparam.setEnabled(false);
			paramtable = new Table(paramgp, SWT.BORDER | SWT.V_SCROLL
					| SWT.FULL_SELECTION);
			...
			removeFn.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					boolean re = MessageDialog.openConfirm(getContainer()
							.getShell(), Messages
							.getString("editors.FnEditor.7"), Messages
							.getString("editors.FnEditor.19"));
					if (re) {
						deleteTreeItem();
					}
				}
			...
			addparam.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					...
					item.setText(new String[] {
							"param" + (paramtable.getItemCount() + 1), "" });
			...
			removeparam.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					boolean re = MessageDialog.openConfirm(getContainer()
							.getShell(), Messages
							.getString("editors.FnEditor.20"), Messages
							.getString("editors.FnEditor.21"));
					if (re) {
						deleteTableItem();
					}
				}
				...
				public void handleEvent(Event e) {
					...
					edtext.addListener(SWT.FocusOut, new Listener() {
						public void handleEvent(Event event) {
							...
							edtext.dispose();
							modelmanage.realoadParam(currentTreeItem,
									paramtable);
							setData();
						}
					});
	...
	private void makeActions() {
		...
		addFnAction.setImageDescriptor(ImageDescriptor
				.createFromImage(StudioPlugin.getIcon("add.gif")));

		removeFnAction = new Action() {
			public void run() {
				boolean re = MessageDialog.openConfirm(getContainer()
						.getShell(), Messages.getString("editors.FnEditor.7"),
						Messages.getString("editors.FnEditor.19"));
				if (re) {
					deleteTreeItem();
				}
			}
		...
		removeFnAction.setImageDescriptor(ImageDescriptor
				.createFromImage(StudioPlugin.getIcon("delete.gif")));
		removeFnAction.setEnabled(false);
		...
		viewSourseAction = new Action() {
			public void run() {
				...
				try {
					...
					if (s < 0) {
						s = document.search(0,
								localElement.attributeValue("id"), true, false,
								false);
		...
		viewSourseAction.setImageDescriptor(ImageDescriptor
				.createFromImage(StudioPlugin.getIcon("tag.gif")));
	}
	...
	private void act(IEditorPart paramIEditorPart) {
		...
		if (paramIEditorPart != null) {
			...
			if ((localIContentOutlinePage != null)
					&& (this.mutiPageContentOutlinePage != null))
				this.mutiPageContentOutlinePage
						.setPageActive(localIContentOutlinePage);
		}
	}

	private void fiend() {
		if ((this.mutiPageContentOutlinePage == null)
				|| (this.mutiPageContentOutlinePage.isDisposed()))
	...
	private void deleteTreeItem() {
		try {
			...
			MessageDialog.openError(getSite().getShell().getShell(),
					Messages.getString("editors.FnEditor.23"), es.toString());
		}
	...
	private void deleteTableItem() {
		try {
			...
			MessageDialog.openError(getSite().getShell().getShell(),
					Messages.getString("editors.FnEditor.24"), es.toString());
		}
	}

	public void setData() {
		editor.getDocumentProvider().getDocument(editor.getEditorInput())
				.set(modelmanage.toString());
