...
public class ConditionPropertys implements IPropertys {
	JSONArray data = new JSONArray();
	List<IProperty> reslist = new ArrayList<IProperty>();

	public ConditionPropertys(JSONArray data) {
		if (data != null)
			this.data = data;
	}
	...
	public List<IProperty> getProperty() {
		try {
			...
			for (int i = 0; i < data.length(); i++) {
				...
				proper.setLabel(ConditionPropertySet.getProperLabel(properid));
				res.put(properid, proper);
			}
			for (String properid : ConditionPropertySet.properlist) {
				if (res.containsKey(properid)) {
					reslist.add(res.get(properid));
				} else {
					PropertyElement proper = new PropertyElement();
					proper.setId(properid);
					proper.setLabel(ConditionPropertySet
							.getProperLabel(properid));
					proper.setValue("");
					proper.setInputType(ConditionPropertySet
							.getProperText(properid));
