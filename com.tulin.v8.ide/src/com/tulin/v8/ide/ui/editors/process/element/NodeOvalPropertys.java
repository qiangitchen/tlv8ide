...
public class NodeOvalPropertys implements IPropertys {
	JSONArray data = new JSONArray();
	List<IProperty> reslist = new ArrayList<IProperty>();

	public NodeOvalPropertys(JSONArray data) {
		if (data != null)
			this.data = data;
	}
	...
	public List<IProperty> getProperty() {
		try {
			...
			for (int i = 0; i < data.length(); i++) {
				...
				proper.setLabel(NodeOvalPropertySet.getProperLabel(properid));
				res.put(properid, proper);
			}
			for (String properid : NodeOvalPropertySet.properlist) {
				if (res.containsKey(properid)) {
					reslist.add(res.get(properid));
				} else {
					PropertyElement proper = new PropertyElement();
					proper.setId(properid);
					proper.setLabel(NodeOvalPropertySet
							.getProperLabel(properid));
					proper.setValue("");
					if ("n_p_name".equals(properid)) {
