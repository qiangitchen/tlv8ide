package com.tulin.v8.ide.common;
/*
		...
		public void run() {
			try {
				...
				try {
					...
					String appRoot = StudioConfig.getStudioAppRootPath();
					if (!resourcePath.contains(appRoot) && !resourcePath.contains("/webdesign")
							&& !resourcePath.contains("/flowdesigner")) {
						try {
							String pathend = resourcePath;
							if (pathend.indexOf("?") > 0) {
								pathend = pathend.substring(0, pathend.indexOf("?"));
							}
							if (pathend.indexOf("/") > -1) {
								pathend = pathend.substring(pathend.lastIndexOf("/"));
							}
							if (pathend.indexOf(".") < 0) {
								return;
							}
				...
				return new WebServerContent() {
					...
					public InputStream getInputStream() {
						...
						try {
							...
							ConsoleFactory.printToConsole(e.getMessage(), true, true);
