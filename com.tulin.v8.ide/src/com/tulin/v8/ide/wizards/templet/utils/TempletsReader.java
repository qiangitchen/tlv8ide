...
public class TempletsReader {
	...
	public static Element getConfigElement() throws Exception {
		String templetConfigFile = StudioConfig.getTempletPath()
				+ "/model/config.xml";
		Element root = null;
		File file = new File(templetConfigFile);
		FileInputStream fileiptstream = new FileInputStream(file);
		BufferedReader Strreader = new BufferedReader(new InputStreamReader(
				fileiptstream));
	...
	private static void readChildItem(TreeItem tree, Element root) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (item.elements().size() > 0) {
				root1.setImage(PlatformUI.getWorkbench().getSharedImages()
	...
	public static String readTempletFoldByName(String templet) throws Exception {
		//Element root = getConfigElement();
		//String result = readTempletFoldByNameByEle(root, templet);
		return StudioConfig.getTempletPath() + "/model/" + templet;
	}
	...
	public static String getTempletPageContext(String templet) throws Exception {
		...
		return FileAndString.FileToString(path + "/" + pageName);
	}

	/*
	 * 获取JS模板内容
	 */
	public static String getTempletJsContext(String templet) throws Exception {
		String path = readTempletFoldByName(templet);
		return FileAndString.FileToString(path + "/" + jsName);
	}

	/*
	 * 获取页面模板内容
	 */
	public static String getTempletPageContext(String templet, String pageName)
			throws Exception {
		String path = readTempletFoldByName(templet);
		return FileAndString.FileToString(path + "/" + pageName);
	}
	 ...
	 * 获取JS模板内容
	 */
	public static String getTempletJsContext(String templet, String jsName)
			throws Exception {
		String path = readTempletFoldByName(templet);
		return FileAndString.FileToString(path + "/" + jsName);
	}
