...
public class MobileListdataPermision extends WizardPage {
	...
	public MobileListdataPermision() {
		super("mobileListdataPermision");
	}
	...
	public void createControl(Composite parent) {
		...
		itemconf.setText(Messages.getString("wizardsaction.dataselect.message.dataclumnperm"));
		...
		ptable.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				setTitle(name);
				if (Messages.getString("wizardsaction.dataselect.message.dataclumnperm").equals(name)) {
	...
	private Composite setTreepermision(Composite shell) {
		...
		button0.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				if (dbkey == null || "".equals(dbkey) || table == null || "".equals(table)) {
					...
					TablecellSelectDialog dial = new TablecellSelectDialog(getShell(), dbkey, table);
					int result = dial.open();
					if (IDialogConstants.OK_ID == result) {
		...
		GridData gridl01 = new GridData();
		gridl01.widthHint = 80;
		label01.setLayoutData(gridl01);
		final Text text01 = new Text(composite, SWT.BORDER | SWT.FILL);
		text01.setEditable(false);
		GridData gridt01 = new GridData(SWT.FILL, SWT.CENTER, false, false);
		gridt01.grabExcessHorizontalSpace = true;
		text01.setLayoutData(gridt01);
		Button button01 = new Button(composite, SWT.NONE);
		button01.setText("...");
		GridData gridb01 = new GridData();
		button01.setLayoutData(gridb01);
		button01.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				if (dbkey == null || "".equals(dbkey) || table == null || "".equals(table)) {
					MessageDialog.openError(getShell(), Messages.getString("wizards.dataselect.message.errtitle"), Messages.getString("wizardsaction.dataselect.message.selectdbandtable"));
				} else {
					...
					int result = dial.open();
					if (IDialogConstants.OK_ID == result) {
						text01.setText(dial.getItemsToOpen());
						title = text01.getText();
						setPageCompleteEnable();
					}
				}
			}
	...
	private void setPageCompleteEnable() {
		boolean enable = (dbkey != null && table != null && ID != null && !"".equals(ID) && title != null && text != null
				&& ellip != null);
	...
	public IWizardPage getNextPage() {
		return getWizard().getPage("mobileListPageEnd");
	}
