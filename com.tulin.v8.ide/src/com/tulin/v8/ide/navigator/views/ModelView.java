...
import com.tulin.v8.ide.navigator.views.action.CollapseAllAction;
import com.tulin.v8.ide.navigator.views.action.DeleteObjAction;
import com.tulin.v8.ide.navigator.views.action.NewCSSFileAction;
import com.tulin.v8.ide.navigator.views.action.NewFileAction;
import com.tulin.v8.ide.navigator.views.action.NewFlowDrawAction;
import com.tulin.v8.ide.navigator.views.action.NewFlowFolderAction;
import com.tulin.v8.ide.navigator.views.action.NewFolderAction;
import com.tulin.v8.ide.navigator.views.action.NewFunctionTreeFileAction;
import com.tulin.v8.ide.navigator.views.action.NewHTMLFileAction;
import com.tulin.v8.ide.navigator.views.action.NewJAVAFileAction;
import com.tulin.v8.ide.navigator.views.action.NewJSFileAction;
import com.tulin.v8.ide.navigator.views.action.NewJSPFileAction;
import com.tulin.v8.ide.navigator.views.action.NewTableAction;
import com.tulin.v8.ide.navigator.views.action.NewXMLFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenCSSFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenFlowDrawAction;
import com.tulin.v8.ide.navigator.views.action.OpenFnFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenFunctionTreeFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenHTMLFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenImageFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenJSFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenJSPFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenJavaFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenPageFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenSpringFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenStrutsFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenTableViewFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenTxtFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenXMLFileAction;
import com.tulin.v8.ide.navigator.views.action.OpenfoldAction;
import com.tulin.v8.ide.navigator.views.action.PropertyViewerAction;
import com.tulin.v8.ide.navigator.views.action.RePropertyFlowDrawAction;
import com.tulin.v8.ide.navigator.views.action.RefreshAction;
import com.tulin.v8.ide.navigator.views.action.RenameAction;
import com.tulin.v8.ide.navigator.views.action.ToggleLinkingAction;
...
public class ModelView extends TreeView implements IStatusChangeListener, IViewPartInputProvider, IPackagesViewPart,
	...
	private OpenfoldAction openfoldAction;
	...
	private NewJSFileAction newjsfileaction;
	private NewHTMLFileAction newhtmlfileaction;
	private NewJSPFileAction newjspfileaction;
	private NewCSSFileAction newcssfileaction;
	private NewJAVAFileAction newjavafileaction;
	private NewFunctionTreeFileAction newfunctiontreefileaction;
		...
		if (obj instanceof OracleSource || obj instanceof OracleSequence) {
			...
			if ("process".equals(tr.getTvtype())) {
				...
				if ("TABLE".equals(tr.getName())) {
					...
					newTableAction
							.setImageDescriptor(ImageDescriptor.createFromImage(StudioPlugin.getIcon("view.gif")));
				}
			} else if ("folder".equals(tr.getTvtype())
					|| ("invisible".equals(tr.getParent().getName()) && !"DATA".equals(tr.getName()))) {
				newmenu.add(newfolderaction);
				if (!"BIZ".equals(tr.getBiz()) && !"BIZ".equals(tr.getName())) {
					newmenu.add(newhtmlfileaction);
					newmenu.add(newjspfileaction);
					newmenu.add(newjsfileaction);
					newmenu.add(newcssfileaction);
					newmenu.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS));
					newmenu.add(newfunctiontreefileaction);
				}
				if ("BIZ".equals(tr.getBiz())) {
					newmenu.add(newjavafileaction);
				}
				if ("UI".equals(tr.getBiz())) {
					newmenu.add(newPageAction);
				}
				newmenu.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS));
				NewXMLFileAction newxmlfileaction = new NewXMLFileAction(this.viewer);
				newmenu.add(newxmlfileaction);
			...
			manager.add(openmenu);
			if ("file".equals(tr.getTvtype())) {
				String filePath = tr.getPath();
				if (filePath.endsWith(".java")) {
					OpenJavaFileAction openjavafileaction = new OpenJavaFileAction(this.viewer);
					openmenu.add(openjavafileaction);
				}
				if (StudioUtil.isHTML(filePath)) {
					OpenHTMLFileAction openhtmlfileaction = new OpenHTMLFileAction(this.viewer);
					openmenu.add(openhtmlfileaction);
				}
				if (StudioUtil.isJSP(filePath)) {
					OpenJSPFileAction openjspfileaction = new OpenJSPFileAction(this.viewer);
					openmenu.add(openjspfileaction);
				}
				if (StudioUtil.isTuLinPage(filePath)) {
					OpenPageFileAction openTuLinpagefileaction = new OpenPageFileAction(this.viewer);
					openmenu.add(openTuLinpagefileaction);
				}
				if (filePath.endsWith(".js")) {
					OpenJSFileAction openjsfileaction = new OpenJSFileAction(this.viewer);
					openmenu.add(openjsfileaction);
				}
				if (filePath.endsWith(".css")) {
					OpenCSSFileAction opencssfileaction = new OpenCSSFileAction(this.viewer);
					openmenu.add(opencssfileaction);
				}
				if (StudioUtil.isFun(filePath)) {
					OpenFunctionTreeFileAction openfunctiontreefileaction = new OpenFunctionTreeFileAction(this.viewer);
					openmenu.add(openfunctiontreefileaction);
				}
				if (StudioUtil.isFn(filePath)) {
					OpenFnFileAction openfnfileaction = new OpenFnFileAction(this.viewer);
					openmenu.add(openfnfileaction);
				}
				if (filePath.endsWith("struts.xml")) {
					OpenStrutsFileAction openstrutsfileaction = new OpenStrutsFileAction(this.viewer);
					openmenu.add(openstrutsfileaction);
				}
				if (filePath.endsWith("applicationContext.xml")) {
					OpenSpringFileAction openspringfileaction = new OpenSpringFileAction(this.viewer);
					openmenu.add(openspringfileaction);
				}
				if (filePath.endsWith(".xml")) {
					OpenXMLFileAction openxmlfileaction = new OpenXMLFileAction(this.viewer);
					openmenu.add(openxmlfileaction);
				}
				if (StudioUtil.isImage(filePath)) {
					OpenImageFileAction openimgfileaction = new OpenImageFileAction(this.viewer);
					openmenu.add(openimgfileaction);
				}
				OpenTxtFileAction opentxtfileaction = new OpenTxtFileAction(this.viewer);
				openmenu.add(opentxtfileaction);
			...
			if ("folder".equals(tr.getTvtype())
					|| ("invisible".equals(tr.getParent().getName()) && !"DATA".equals(tr.getName()))) {
				manager.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS));
				manager.add(openfoldAction);
				DeleteAction.setEnabled(true);
			}
		} else {
			manager.add(openAction);
	...
	protected void makeActions() {
		...
		newfolderaction = new NewFolderAction(this.viewer);
		newflowfolderaction = new NewFlowFolderAction(this.viewer);
		newflowdrawaction = new NewFlowDrawAction(this.viewer);
		newfileaction = new NewFileAction(this.viewer);
		newjsfileaction = new NewJSFileAction(this.viewer);
		newhtmlfileaction = new NewHTMLFileAction(this.viewer);
		newjspfileaction = new NewJSPFileAction(this.viewer);
		newcssfileaction = new NewCSSFileAction(this.viewer);
		newjavafileaction = new NewJAVAFileAction(this.viewer);
		newfunctiontreefileaction = new NewFunctionTreeFileAction(this.viewer);
		...
		newPageAction = new Action() {
			public void run() {
				// 新建页面
				Shell shell = StudioPlugin.getShell();
				SampleNewWizard wizard = new SampleNewWizard(viewer.getSelection());
		...
		newPageAction.setText(Messages.getString("ModelView.newPageAction.1"));
		newPageAction.setToolTipText(Messages.getString("ModelView.newPageAction.2"));
		newPageAction.setImageDescriptor(ImageDescriptor.createFromImage(StudioPlugin.getIcon("page.gif")));

		openfoldAction = new OpenfoldAction(this.viewer);
