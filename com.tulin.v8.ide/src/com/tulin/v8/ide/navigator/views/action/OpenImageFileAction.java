...
public class OpenImageFileAction extends Action implements Runnable {
	TreeViewer viewer;

	public OpenImageFileAction(TreeViewer viewer) {
		this.viewer = viewer;
		setText(Messages.getString("View.Action.openfile.27"));
		setText(Messages.getString("View.Action.openfile.28"));
		setImageDescriptor(ImageDescriptor.createFromImage(StudioPlugin.getIcon("image.gif")));
	}
			...
			if (fle.isFile()) {
				try {
					...
					try {
						...
						IDE.openEditor(page, localFileEditorInput2, "com.TuLin.imageviewer.editors.ImageViewEditor");
					} catch (Exception e) {
