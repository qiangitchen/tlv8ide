...
public class OpenStrutsFileAction extends Action implements Runnable {
	TreeViewer viewer;

	public OpenStrutsFileAction(TreeViewer viewer) {
		this.viewer = viewer;
		setText(Messages.getString("editor.name.struts"));
				...
				try {
					...
					try {
						FileEditorInput localFileEditorInput2 = new FileEditorInput(
								iFile);
						IDE.openEditor(page, localFileEditorInput2, StrutsConfigEditor.ID);
					} catch (Exception e) {
