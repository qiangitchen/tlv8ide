 ...
 * 
 * the selection is read and written to the file ".webclasspath" in the root of
 * the project.
 * 
 * A special WebAppClassLoader implementation for tomcat 4.x loads the generated
 * file during startup of the webapplication.
 * 
 * @version 1.0
...
public class TomcatProjectWebclasspathPropertyPage {
	...
	public boolean performOk() {
		...
		try {
			if (webClassPathCheck.getSelection() == true) {
				page.getTomcatProject().setWebClassPathEntries(
						new WebClassPathEntries(newSelection));
			} else {
	...
	public Control getControl(Composite ctrl) {
		...
		webClassPathCheck
				.setText(TomcatPluginResources.PROPERTIES_PAGE_PROJECT_ACTIVATE_DEVLOADER_LABEL);
		...
		cpList = new CheckedListDialogField(null, new String[] { "Check All",
				"Uncheck All" }, new LabelProvider() {
		});
		...
		if (entries != null) {
			...
			// check for entries which are still in the list but no more in
			// classpath entries list and remove them
		...
		/*
		 * Quick hack : Using reflection for compatability with Eclipse 2.1 and
		 * 3.0 M9
		 * 
		 * Old code : cpList.setElements(classPathEntries); if (entries != null)
	...
	private void getClassPathEntries(IClasspathEntry[] entries,
			IJavaProject prj, ArrayList data, IPath outputPath) {
		for (int i = 0; i < entries.length; i++) {
			...
			if (entry.getEntryKind() == IClasspathEntry.CPE_PROJECT) {
				...
				if (!visitedProjects.contains(prjName)) {
					...
					getClassPathEntries(getJavaProject().getJavaModel()
							.getJavaProject(prjName), data);
				}
				...
				if (!entry.getPath().toString()
						.equals("org.eclipse.jdt.launching.JRE_CONTAINER")) {
					// Add container itself, as TomcatBootstrap can actually
					// process them
					// at the moment
					// Basically, users will be able to choose b/w the whole
					// container
					...
					try {
						...
						TomcatLauncherPlugin
								.log("failed to obtain classpath container '"
										+ entry.getPath() + "'"
										+ " for project '"
					...
					if (container != null) {
						getClassPathEntries(container.getClasspathEntries(),
								prj, data, outputPath);
					}
				}
			} else {
	...
	private void add(ArrayList data, IPath entry) {
		String path = CommonUtil.rebuildFilePath(entry.toFile().toString());
		// ignore tomcat's own libs and the JRE paths..
		if (!data.contains(path) && path.indexOf("TOMCAT_HOME") == -1
				&& path.indexOf("JRE_CONTAINER") == -1
				&& path.indexOf("JRE_LIB") == -1) {
			data.add(path);
		}
	}
	...
	private java.util.List readSelectedEntries() {
		...
		IFile file = getJavaProject().getProject().getFile(
				new Path(WEBAPP_CLASSPATH_FILENAME));
		if (file == null)
			return selected;
		...
		if (cpFile.exists()) {
			...
			try {
				...
				TomcatLauncherPlugin.logException(ioEx);
			} finally {
				if (reader != null)
					try {
						reader.close();
