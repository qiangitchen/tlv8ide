...
public class TomcatJVMPreferencePage extends PreferencePage implements
	...
	protected Control createContents(Composite parent) {
		...
		jvmChoice = new ComboFieldEditor(
				TomcatLauncherPlugin.TOMCAT_PREF_JRE_KEY, PREF_PAGE_JRE_LABEL,
				namesAndValues, composite);

		debugModeEditor = new BooleanFieldEditor(
				TomcatLauncherPlugin.TOMCAT_PREF_DEBUGMODE_KEY,
		...
		btAddLaunch.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				try {
					TomcatLauncherPlugin.getDefault().getTomcatBootstrap()
							.addLaunch();
				} catch (Exception ex) {
		...
		jvmParamaters = new ListFieldEditor(
				TomcatLauncherPlugin.TOMCAT_PREF_JVM_PARAMETERS_KEY,
				PREF_PAGE_PARAMETERS_LABEL, composite);
		jvmClasspath = new ClasspathFieldEditor(
				TomcatLauncherPlugin.TOMCAT_PREF_JVM_CLASSPATH_KEY,
				PREF_PAGE_CLASSPATH_LABEL, composite);
		jvmBootClasspath = new ClasspathFieldEditor(
				TomcatLauncherPlugin.TOMCAT_PREF_JVM_BOOTCLASSPATH_KEY,
