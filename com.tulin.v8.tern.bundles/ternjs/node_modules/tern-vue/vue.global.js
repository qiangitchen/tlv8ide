...
(function(mod) {
  ...
  function globalInclude(name) {
    var service = infer.cx().definitions.vue.service;
    if (service.hasProp(name)) return service.getProp(name);
    var provider = infer.cx().definitions.vue.provider;
  ...
  function applyWithInjection(mod, fnType, node, asNew) {
    ...
    if (/FunctionExpression/.test(node.type)) {
      for (var i = 0; i < node.params.length; ++i) {
        ...
        elt.vue = {type: "field", parentModule: mod, field: dep};
      }
  ...
  function getModule(name) {
    return infer.cx().parent.mod.vue.modules[name];  
  }  
  ...
  var AngularElement = function(name, originNode, fnType, kind, parent) {
    this.originNode = originNode; // origine node for the name of the vue element
    this.fnType = fnType; // function type of the vue element
  ...
  function declareMod(name, includes, node) {
    var cx = infer.cx(), data = cx.parent.mod.vue;
  ...
  infer.registerFunction("angular_module", function(_self, _args, argNodes) {
    ...
    // mark node as module (when vue.module() => argNodes[0] is null)
    if (argNodes[0]) argNodes[0].vue = {type: "module"};
    // include the module
    if (mod) mod.include();
    return mod;
  });
  ...
  function baseUrl() {
    ...
    return server.mod.vue.options.baseURL || "";    
  }
  ...
  infer.registerFunction("angular_templateUrl", function(argN) {
    return function(self, args, argNodes) {
      ...
      if (node && node.type == "Literal" && typeof node.value == "string") {        
        ...
        node.vue = {type: "templateUrl", templateUrl: resolveProjectPath(node.value)};
      }
  ...
  function postLoadDef(json) {
    ...
    if (defName == "vue") {
      var proto = moduleProto(cx), naked = cx.parent.mod.vue.nakedModules;
  ...
  function preCondenseReach(state) {
    var mods = infer.cx().parent.mod.vue.modules;
  ...
  function getTypeAt(expr) {
    if (!expr || !expr.node || !expr.node.vue) return null;
    var data = expr.node.vue;
  ...
  function typeAtTemplateUrl(file, pos, expr, type) {
    if (!(expr.node.type === "Literal" && typeof expr.node.value === "string")) return;
    type = Object.create(type);
    type.origin = expr.node.vue.templateUrl;
    return type;
  }

  function typeAtField(file, pos, expr, type) {
    if (!(expr.node.type === "Literal" && typeof expr.node.value === "string")) return;
    var fieldType = expr.node.vue.field && expr.node.vue.field.getType();
    type = Object.create(type);
    copyTypeInfo(fieldType, type);
    return type;
  }
  ...
  function completeModuleName(query, file, node, word) {
    var completions = [];
    var cx = infer.cx(), server = cx.parent, data = server.mod.vue;
  ...
  function completeInjectionParam(query, file, node, word) {
    var completions = [], mod = node.vue && node.vue.parentModule, fields = mod && mod.injector && mod.injector.fields;
    if (!fields) return;
    var cx = infer.cx(), vue = cx.definitions.vue;
    ...
    if (query.caseInsensitive) word = word.toLowerCase();    
    gather(vue.service.props);
    gather(vue.provider.props);
    gather(fields, true);
    return completions;
  }
    
  function preInfer(ast, scope) {
    var filename = ast.sourceFile.name, server = infer.cx().parent, vue= server.mod.vue, mods = vue.modules;
    // marks the vue modules of the current file as excluded.
  ...
  function postInfer(ast, scope) {
    ...
    function updateOutline(ast) {
      var filename = ast.sourceFile.name, server = infer.cx().parent, vue = server.mod.vue, mods = vue.modules, changes = false;
      for (var name in mods) {
        if (mods[name].update(filename)) changes = true;
      }
      if (!server.sendToClient) return;
      if (changes || !vue.outline) return createOutline(server);
      var oldOutline = vue.outline, newOutline = createOutline(server);
      if (JSON.stringify(oldOutline) != JSON.stringify(newOutline)) return newOutline;
    }
    
    // delete the vue modules of the current file which are marked as excluded.
    var outline = updateOutline(ast);
    if (outline) {
      // outline has changed
      var server = infer.cx().parent, vue = server.mod.vue;
      vue.outline = outline;
      server.sendToClient("vue:modelChanged", outline);
    }        
  }
  ...
  var defs = {
    ...
    "!define": {
      ...
      Module: {
        "!url": "http://docs.angularjs.org/api/vue.Module",
        "!doc": "Interface for configuring vue modules.",
        prototype: {
          animation: {
            "!type": "fn(name: string, animationFactory: fn()) -> !this",
            "!url": "http://docs.angularjs.org/api/vue.Module#animation",
          ...
          config: {
            ...
            "!url": "https://docs.angularjs.org/api/ng/type/vue.Module#config",
          ...
          run: {
            ...
            "!effects": ["custom angular_callInject 0"],
            "!url": "http://docs.angularjs.org/api/vue.Module#run",
          ...
          name: {
            "!type": "string",
            "!url": "http://docs.angularjs.org/api/vue.Module#name",
            "!doc": "Name of the module."
          },
          requires: {
            "!type": "[string]",
            "!url": "http://docs.angularjs.org/api/vue.Module#requires",
      ...
      service: {
        ...
        $exceptionHandler: {
          ...
          "!doc": "Any uncaught exception in vue expressions is delegated to this service."
        },
        $filter: {
        ...
        $parse: {
          ...
          "!doc": "Converts vue expression into a function."
        },
        $q: {
        ...
        $rootElement: {
          ...
          "!doc": "The root element of vue application."
        },
        $rootScope: {
          ...
          $apply: {
            ...
            "!doc": "Execute an expression in vue from outside of the vue framework."
          },
          $broadcast: {
        ...
        $timeout: {
          ...
          "!doc": "vue's wrapper for window.setTimeout.",
        ...
        $provide: {
          ...
          constant: {
            ...
            "!doc": "Register a constant service, such as a string, a number, an array, an object or a function, with the $injector. Unlike value it can be injected into a module configuration function (see vue.Module) and it cannot be overridden by an vue decorator."
          },
          decorator: {
      ...
      provider: {
        ...
        $controllerProvider: {
          ...
          "!doc": "The $controller service is used by vue to create new controllers. This provider allows controller registration via the register method.",
          register: {
            "!type": "fn(name: string, constructor: fn()|[]) -> !this",
  ...
  tern.defineQueryType("vue", {
    run : function(server, query) {
      ...
      var _angular = server.cx.parent.mod.vue;
  ...
  // vue "outline" query type
  
  function createOutline(server) {
    var vue = server.mod.vue, mods = vue.modules, outline = [];
    for (var name in mods) mods[name].toJSON(outline);
    return {outline: outline};
  }

  tern.defineQueryType("vue-outline", {
    run : function(server, query) {
