		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				TernToolsUIPlugin
						.getDefault()
						.getLog()
						.log(new Status(IStatus.ERROR, TernToolsUIPlugin.PLUGIN_ID,
								s, t));
				logged.add(s);
			}
		}

		if (!TernToolsUIPlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(TernToolsUIPlugin.PLUGIN_ID);
		sb.append(" ");
