...
import com.tulin.v8.fn.FnEditor;
import com.tulin.v8.function.editors.FunctionEditor;
import com.tulin.v8.ide.StudioPlugin;
import com.tulin.v8.ide.internal.FlowDraw;
import com.tulin.v8.ide.utils.StudioUtil;
import com.tulin.v8.ide.views.navigator.job.OpenTableViewEditorJob;
...
public class OpenFileAction extends Action implements Runnable {
	...
	public OpenFileAction(TreeViewer viewer) {
		...
		setImageDescriptor(ImageDescriptor
				.createFromImage(PlatformUI.getWorkbench().getSharedImages().getImage(ISharedImages.IMG_OBJ_FILE)));
	}
	...
	private void openFile() {
		...
		if (element instanceof DataBase) {
			...
			// 打开表&视图编辑器
			OpenTableViewEditorJob job = new OpenTableViewEditorJob(viewer, (ITable) element);
			...
			if (containerName != null && !"".equals(containerName)) {
				...
				if (fle.isFile()) {
					try {
						...
						String editorid = getEditorID(containerName, treeobj.getBiz());
						if (iFile != null && iFile.exists()) {
							...
							if (editorid != null) {
								try {
									...
									IDE.openEditor(page, localFileEditorInput2, editorid);
								} catch (Exception e) {
									Sys.packErrMsg(e.toString());
								}
							} else {
							...
							if (editorid != null) {
								...
								FileStoreEditorInput localFileStoreEditorInput = new FileStoreEditorInput(
										localLocalFile);
								IDE.openEditor(page, localFileStoreEditorInput, editorid);
							} else {
								IFileStore fileStore = EFS.getLocalFileSystem().getStore(new Path(containerName));
	...
	private String getEditorID(String containerName, String bizName) {
		if (containerName.toLowerCase().endsWith(".fun.xml")) {
			...
			return FnEditor.ID;
		} else if (StudioUtil.isTuLinPage(containerName) && !"mobileUI".equals(bizName)) {
