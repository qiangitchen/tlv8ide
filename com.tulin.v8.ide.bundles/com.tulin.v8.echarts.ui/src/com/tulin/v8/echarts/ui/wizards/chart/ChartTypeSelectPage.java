...
//					chart.setLayoutData(rowlayoutd);
					String imagePath = "/chartsnewizard/image/emptyPage.png";
					Element el = (Element) item.elements().get(m);
					if (!"".equals(el.attributeValue("icon")) && el.attributeValue("icon") != null) {
						imagePath = "/chartsnewizard/" + el.attributeValue("icon");
					}
					chart.setData(el);
					chart.setText(el.attributeValue("label"));
					chart.setImage(Activator.getDefault().getImage(imagePath));
