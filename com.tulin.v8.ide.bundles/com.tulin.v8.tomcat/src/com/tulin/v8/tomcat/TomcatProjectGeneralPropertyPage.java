...
public class TomcatProjectGeneralPropertyPage implements TomcatPluginResources {
	...
	public void createIsTomcatProjectGroup(Composite parent) {
		...
		isTomcatProjectGroup.setLayoutData(new GridData(
				GridData.FILL_HORIZONTAL));

		// project location entry field
		isTomcatProjectCheck = new Button(isTomcatProjectGroup, SWT.CHECK
				| SWT.LEFT);
		isTomcatProjectCheck
				.setText(PROPERTIES_PAGE_PROJECT_ISTOMCATPROJECT_LABEL);
		isTomcatProjectCheck.setEnabled(true);

		try {
			isTomcatProjectCheck.setSelection(page.getJavaProject()
					.getProject().hasNature(TomcatLauncherPlugin.NATURE_ID));
		} catch (CoreException ex) {
			TomcatLauncherPlugin.logError(ex.getMessage());
		}
	}
	...
	public void createExtraInformationGroup(Composite parent) {
		...
		extraInfoText = new Text(contextGroup, SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		GridData data = new GridData(GridData.FILL_HORIZONTAL);
