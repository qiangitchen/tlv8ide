...
/**
 * @name checkEncode
 * @description 检查页面编码 如果没有配置自动设置为utf-8
 * @returns {boolean}
 */
...
/**
 * @name tlv8.RequestURLParam
 * @description URL参数处理类
 */
tlv8.RequestURLParam = {
	...
	/**
	 * @name tlv8.RequestURLParam.getParamByURL
	 * @description 根据指定的URL地址，获取URL参数
	 * @param {string}
	 *            paramName
	 * @returns {string}
	 */
...
tlv8.Data = function() {
	...
	this.saveData = function() {
		...
		for (var i = 0; i < inputs.length; i++) {
			if (inputs[i].type == "text" || inputs[i].type == "textarea"
				...
				if ($rid && $rid != ""
						...
						&& $rid.indexOf("_editgridipt") < 0) {
					inputs[i].title = $rval;
					inputs[i].setAttribute("value", $rval);
		...
		for (var i = 0; i < textareas.length; i++) {
			...
			if ($rid && $rid != ""
					&& $rid.toUpperCase().substr(4, 12) != "_FIXFFCURSOR") {
				textareas[i].title = $rval;
				textareas[i].value = $rval;
				textareas[i].setAttribute("value", $rval);
		...
		for (var i = 0; i < selects.length; i++) {
			var $rid = selects[i].id;
			var $rval = $(selects[i]).val();
			if ($rid && $rid != ""
					&& $rid.toUpperCase().substr(4, 12) != "_FIXFFCURSOR") {
				selects[i].title = $rval;
				selects[i].setAttribute("value", $rval);
...
tlv8.toolbar = function(div, insertitem, saveitem, deleteitem, refreshitem) {
	...
	var Stander = "<table style='align:left;' class='standard_toolbar' border='0' id='toolbar'><tr><td width='54px' align='left' id='"
			+ div.id
			...
			+ $dpimgpath
			+ "toolbar/refreshbill.gif'/></a></td></tr></table>";
		...
		try {
			document.getElementById("dailogenginItem").onclick = function() {
				...
				if (dailogEngin) {
					...
					if (reData == false)
						return;
					tlv8.portal.dailog.dailogEngin(reData, true);
				} else {
					tlv8.portal.dailog.dailogEngin(null, true);
				}
			};
		} catch (e) {
		}
...
tlv8.ExcelImp = function(dbkey, table, relation, confirmXmlName, callback) {
	...
	tlv8.portal.dailog.openDailog('Excel导入', url, 300, 320, callback,
			false);
};
tlv8.ExcelExp = function(dbkey, table, relation, labels, where, orderby) {
	...
	tlv8.portal.dailog.openDailog('导出Excel', url, 270, 300, null, false,
			null, params);
};
...
tlv8.standardPartition = function(div) {
	if (!div)
		return;
	if (div.tagName != "DIV")
		return;
	if($(div).layout){
		...
		$(div).layout('add',{    
		    region: 'west',    
		    width: $(div).find("div[stpa='left']").width(),    
		    split: true,
		    border: false,
		    content: $(div).find("div[stpa='left']").children()
		});  
		$(div).layout('add',{    
			region: 'center',
			border: false,
			content: $(div).find("div[stpa='right']").children()
		});
		$(div).find("div[stpa='left']").remove();
		$(div).find("div[stpa='right']").remove();
		return;
	}
	var spheight = "100%";
	...
	spTable += "<td width='6px' onmouseover='tlv8.splitMove(this)' id='"
			+ div.id
...
tlv8.fileComponent = function(div, data, cellname, docPath, canupload,
	...
	try{
		...
		createStyleSheet($rp+"resources/layui/layuiadmin/layui/css/layui.css");
		createJSSheet($rp+"resources/layui/layuiadmin/layui/layui.all.js");
	}
	var $commonpath = $dpjspath.replace("/js/", "/");
...
tlv8.trangereditfile = function(fileID, fileName, docPath, dbkey,
	...
	tlv8.portal.openWindow("文件" + fileName + "编辑",
			"/comon/doc_ocx/tangerOffice/officeediter.jsp?fileID="
...
// X5文件编辑器编辑
tlv8.editfile = function(fileID, fileName, docPath) {
	var OVP = {
		host : docPath,
		userName : tlv8.Context.getCurrentPersonName(),
		userInitials : tlv8.Context.getCurrentPersonName(),
	...
	var afterEditOfficeDialogSelect = function(data) {
		if (data != '' && data.changes != 'W10=') {
			...
			if (data != '' && data.changes == 'W10=' && data.createVersion) {
				tlv8.createVersion(docID, fileID, data.filename, docPath);
			} else {
				var currentNode = justep.Doc.evalChangeLog(tlv8.changeLog,
						docID);
	...
	if($.messager){
		...
		if (!state) {
			...
			try{
				...
				var leftW = (width - 100) / 2;
				var top = (height - 100) / 2;
				msg = msg || "正在加载，请稍候... ...";
				var _html = "<div id='s1loading' style='position:absolute;left:0;width:100%;height:"
						+ height
						+ "px;top:0;background:#eee;opacity:0.7;filter:alpha(opacity=70);'><div style='position:absolute;  cursor1:wait;left:"
						+ leftW
						+ "px;top:"
						+ top
						+ "px;width:auto;height:18px;font-size:12px;padding:10px 5px 10px 30px;background:#fff url("+cpath+"/resources/jquery-easyui/themes/default/images/loading.gif) no-repeat scroll 5px 10px;border:2px solid #ccc;color:#000;'>"+msg+"</div></div>";
...
tlv8.loadOption = function(viewID, sData1) {
	...
	var viewHTml = "<ul style='width:100%;'>";
	var redata = re.data;
	for (var i = 0; i < redata.length; i++) {
		...
		var pcre = tlv8.sqlQueryAction("system",
...
// $(function(){
// if($.messager){
// window.alert = function(msg, title, icon, cals){
// $.messager.alert(title||'提示', msg, icon, cals);
// };
// }
// });
