...
public class ImageView extends ViewPart implements ISelectionListener {
		...
		try {
			...
			for (int i = 0; i < resources.length; i++) {
				if (resources[i] instanceof IFile) {
					...
					for (int j = 0; j < WebToolsPlugin.SUPPORTED_IMAGE_TYPES.length; j++) {
						if (file.getFileExtension().equalsIgnoreCase(WebToolsPlugin.SUPPORTED_IMAGE_TYPES[j])) {
							addImage(file);
							break;
						}
					}
	...
	private void addImage(IFile file) {
		try {
			...
			item.setImage(i);
			item.setText(
					file.getName() + " (" + image.getImageData().width + " * " + image.getImageData().height + ")");
			imageList.add(image);
			iconList.add(i);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public void createPartControl(Composite parent) {
		...
		label.setText(WebToolsPlugin.getResourceString("ImageView.Preview"));
