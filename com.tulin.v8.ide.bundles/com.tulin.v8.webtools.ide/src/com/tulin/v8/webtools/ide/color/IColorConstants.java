package com.tulin.v8.webtools.ide.color;

import org.eclipse.swt.graphics.RGB;

/** Defines initial colors used in editors. */
public interface IColorConstants {
	RGB HTML_COMMENT = new RGB(63, 95, 191);
	RGB PROC_INSTR = new RGB(128, 128, 128);
	RGB STRING = new RGB(42, 0, 255);
	RGB DEFAULT = new RGB(0, 0, 0);
	RGB TAG = new RGB(63, 127, 127);
	RGB ATTR = new RGB(127, 0, 127);
	RGB VALUE = new RGB(42, 0, 255);
	RGB SCRIPT = new RGB(191, 95, 63);
	RGB CSS_COMMENT = new RGB(63, 95, 191);
	RGB CSS_PROP = new RGB(127, 0, 127);
	RGB CSS_VALUE = new RGB(42, 0, 225);
	RGB FOREGROUND = new RGB(0, 0, 0);
	RGB BACKGROUND = new RGB(255, 255, 255);
	RGB JAVA_COMMENT = new RGB(63, 127, 95);
	RGB JAVA_STRING = new RGB(0, 0, 255);
	RGB JAVA_KEYWORD = new RGB(127, 0, 85);
	RGB TAGLIB = new RGB(191, 95, 63);
	RGB TAGLIB_ATTR = new RGB(128, 128, 0);
	RGB JSDOC = new RGB(63, 95, 191);
}
