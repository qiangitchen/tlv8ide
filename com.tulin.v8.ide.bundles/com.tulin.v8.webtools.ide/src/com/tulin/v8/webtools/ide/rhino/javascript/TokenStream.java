...
/**
 * This class implements the JavaScript scanner. It is based on the C source
 * files jsscan.c and jsscan.h in the jsref package.
 */
class TokenStream {
	/*
	 * For chars - because we need something out-of-range to check. (And checking
	 * EOF by exception is annoying.) Note distinction from EOF token type!
	 */
...
// The following assumes that Token.EOF == 0
		final int Id_break = Token.BREAK, Id_case = Token.CASE, Id_continue = Token.CONTINUE,
				...
				Id_if = Token.IF, Id_in = Token.IN, Id_let = Token.LET, Id_new = Token.NEW, Id_null = Token.NULL,
				...
				Id_with = Token.WITH, Id_yield = Token.YIELD,

				// the following are #ifdef RESERVE_JAVA_KEYWORDS in jsscan.c
				Id_abstract = Token.RESERVED, Id_boolean = Token.RESERVED, Id_byte = Token.RESERVED,
				Id_catch = Token.CATCH, Id_char = Token.RESERVED, Id_class = Token.RESERVED, Id_const = Token.CONST,
				Id_debugger = Token.DEBUGGER, Id_double = Token.RESERVED, Id_enum = Token.RESERVED,
...
// #/string_id_map#
		...
		retry: for (;;) {
			...
			if (isDigit(c) || (c == '.' && isDigit(peekChar()))) {
				...
				if (base == 16) {
					...
					while ('0' <= c && c <= '9') {
						/*
						 * We permit 08 and 09 as decimal numbers, which makes our behavior a superset
						 * of the ECMA numeric grammar. We might not always be so permissive, so we warn
						 * about it.
						 */
						if (base == 8 && c >= '8') {
			...
			case '/':
				...
				if (matchChar('/')) {
					...
					Node commentNode = new Node(Token.COMMENT);
					commentNode.sourceStart = sourceCursor - 2;
					skipLine();
					// MODIFIED BY AMATERAS
					commentNode.sourceEnd = sourceCursor;
					...
					for (;;) {
						...
						if (c == EOF_CHAR) {
							...
							if (lookForSlash) {
								// MODIFIED BY AMATERAS
								if (isJsDoc) {
									jsDocNode = new Node(Token.JSDOC);
									jsDocNode.sourceStart = start;
									jsDocNode.sourceEnd = sourceCursor;
									commentList.add(jsDocNode);
								} else {
									Node commentNode = new Node(Token.COMMENT);
									commentNode.sourceStart = start;
									commentNode.sourceEnd = sourceCursor;
									commentList.add(commentNode);
								}
								continue retry;
	 ...
	 * note that code in getChar() implicitly accepts '\r' == \u000D as well.
	 */
				...
				case '<':
					...
					case '!':
						...
						case '[':
							c = getChar(); // Skip [
							addToString(c);
							if (getChar() == 'C' && getChar() == 'D' && getChar() == 'A' && getChar() == 'T'
									&& getChar() == 'A' && getChar() == '[') {
								addToString('C');
								addToString('D');
								addToString('A');
	...
	public List<Node> commentList = new ArrayList<Node>();
}
