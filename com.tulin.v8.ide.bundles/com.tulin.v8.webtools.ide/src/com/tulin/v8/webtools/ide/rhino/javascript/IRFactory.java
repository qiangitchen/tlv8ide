...
//        return Node.newString(Token.NAME, name);
		...
		node.sourceStart = parser.ts.sourceCursor;
		return node;
	}
	...
	Node createString(String string) {
		...
		// MODIFIED BY AMATERAS
		Node node = Node.newString(string);
		node.sourceStart = parser.ts.sourceCursor;
		return node;
	}
	 ...
	 * @param catchCond the condition under which to catch the exception. May be
	 *                  null if no condition is given.
	...
	/**
	 * Statement block Creates the empty statement block Must make subsequent calls
	 * to add statements to the node
	 */
	Node createBlock(int lineno) {
		// return new Node(Token.BLOCK, lineno);
		// MODIFIED BY AMATERAS
		Node node = new Node(Token.BLOCK, lineno);
		node.sourceStart = parser.ts.sourceCursor;
		return node;
	}
	...
	Node initFunction(FunctionNode fnNode, int functionIndex, Node statements, int functionType) {
		...
		if (functionType == FunctionNode.FUNCTION_EXPRESSION) {
			...
			if (name != null && name.length() != 0) {
				...
				Node setFn = new Node(Token.EXPR_VOID,
						new Node(Token.SETNAME, Node.newString(Token.BINDNAME, name), new Node(Token.THISFN)));
		...
		// MODIFIED BY AMATERAS
//        result.sourceStart = parser.ts.sourceCursor;
		result.fnNode = fnNode;
		result.fnNode.sourceEnd = parser.ts.sourceCursor;
		result.putIntProp(Node.FUNCTION_PROP, functionIndex);
		return result;
	}

	/**
	 * Add a child to the back of the given node. This function breaks the Factory
	 * abstraction, but it removes a requirement from implementors of Node.
	 */
	...
	/**
	 * Create a node that can be used to hold lexically scoped variable definitions
	 * (via let declarations).
	 * 
	...
	Node createFor(Node loop, Node init, Node test, Node incr, Node body) {
		if (init.getType() == Token.LET) {
			...
			let.addChildToBack(createLoop((Node.Jump) loop, LOOP_FOR, body, test, new Node(Token.EMPTY), incr));
			return let;
		}
	...
	Node createForIn(int declType, Node loop, Node lhs, Node obj, Node body, boolean isForEach) {
		...
		if (type == Token.VAR || type == Token.LET) {
			...
			if (lhs.getFirstChild() != lastChild) {
				/*
				 * check that there was only one variable given. we can't do this in the parser,
				 * because then the parser would have to know something about the 'init' node of
				 * the for-in loop.
				 */
				parser.reportError("msg.mult.index");
			}
			if (lastChild.getType() == Token.ARRAYLIT || lastChild.getType() == Token.OBJECTLIT) {
				type = destructuring = lastChild.getType();
				lvalue = lastChild;
				destructuringLen = lastChild.getIntProp(Node.DESTRUCTURING_ARRAY_LENGTH, 0);
		...
		int initType = (isForEach) ? Token.ENUM_INIT_VALUES
				: (destructuring != -1) ? Token.ENUM_INIT_ARRAY : Token.ENUM_INIT_KEYS;
	 ...
	 * Try/Catch/Finally
	 *
	 * The IRFactory tries to express as much as possible in the tree; the
	 * responsibilities remaining for Codegen are to add the Java handlers: (Either
	 * (but not both) of TARGET and FINALLY might not be defined)
	 * 
	 * - a catch handler for javascript exceptions that unwraps the exception onto
	 * the stack and GOTOes to the catch target
	 * 
	 * - a finally handler
	 * 
	...
	/**
	 * Object Literals <BR>
	 * createObjectLiteral rewrites its argument as object creation plus object
	 * property entries, so later compiler stages don't need to know about object
	 * literals.
	 */
...
//                object.addChildToBack(value);
				// MODIFIED BY AMATERAS
				Node propNode = Node.newString(Token.OBJPROP, properties[i].toString());
				propNode.addChildToBack(value);
				object.addChildToBack(propNode);
			}
		}
	...
	/**
	 * Given a destructuring assignment with a left hand side parsed as an array or
	 * object literal and a right hand side expression, rewrite as a series of
	 * assignments to the variables defined in left from property accesses to the
	 * expression on the right.
	 * 
	 * @param type  declaration type: Token.VAR or Token.LET or -1
	 * @param left  array or object literal containing NAME nodes for variables to
	 *              assign
	 * @param right expression to assign from
	 * @return expression that performs a series of assignments to the variables
	 *         defined in left
	 */
	...
	private Node destructuringAssignmentHelper(int variableType, Node left, Node right, String tempName) {
		Node result = createScopeNode(Token.LETEXPR, parser.getCurrentLineNumber());
		...
		final int setOp = variableType == Token.CONST ? Token.SETCONST : Token.SETNAME;
		...
		if (type == Token.ARRAYLIT) {
			...
			for (;;) {
				...
				if (n.getType() == Token.NAME) {
					...
					comma.addChildToBack(new Node(setOp, createName(Token.BINDNAME, name, null), rightElem));
					if (variableType != -1) {
						parser.defineSymbol(variableType, true, name);
						destructuringNames.add(name);
					}
				} else {
					comma.addChildToBack(destructuringAssignmentHelper(variableType, n, rightElem,
			...
			int index = 0;
			Object[] propertyIds = (Object[]) left.getProp(Node.OBJECT_IDS_PROP);
			for (Node n = left.getFirstChild(); n != null; n = n.getNext()) {
				Object id = propertyIds[index];
				Node rightElem = id instanceof String
						? new Node(Token.GETPROP, createName(tempName), createString((String) id))
						: new Node(Token.GETELEM, createName(tempName), createNumber(((Number) id).intValue()));
				if (n.getType() == Token.NAME) {
					String name = n.getString();
					comma.addChildToBack(new Node(setOp, createName(Token.BINDNAME, name, null), rightElem));
					if (variableType != -1) {
	...
	private void checkActivationName(String name, int token) {
		if (parser.insideFunction()) {
			...
			if ("arguments".equals(name) || (parser.compilerEnv.activationNames != null
