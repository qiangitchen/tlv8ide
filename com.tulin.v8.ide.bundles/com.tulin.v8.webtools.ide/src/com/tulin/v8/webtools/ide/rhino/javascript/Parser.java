...
//		++syntaxErrorCount;
	 ...
	 * will be returned. (The parse failure will result in a call to the
...
//        }
		...
		// MODIFIED BY AMATERAS
		currentScriptOrFn.commentList = ts.commentList;

		return currentScriptOrFn;
	}

	/*
	 * The C version of this function takes an argument list, which doesn't seem to
	 * be needed for tree generation... it'd only be useful for checking argument
	 * hiding, which I'm not doing anyway...
	 */
	private Node parseFunctionBody() throws IOException {
		...
		// MODIFIED BY AMATERAS
		pn.sourceEnd = ts.sourceCursor - 1;
		return pn;
	}
	...
	private Node function(int functionType) throws IOException, ParserException {
		...
		// MODIFIED BY AMATERAS
		fnNode.sourceStart = ts.sourceCursor - name.length() - 1;
		if (ts.jsDocNode != null) {
			fnNode.jsDocNode = ts.jsDocNode;
		}
		ts.jsDocNode = null;
				...
				do {
					...
					if (tt == Token.LB || tt == Token.LC) {
						...
						destructuring.addChildToBack(
								nf.createDestructuringAssignment(Token.VAR, primaryExpr(), nf.createName(parmName)));
					} else {
	...
	private Node statementHelper(Node statementLabel) throws IOException, ParserException {
		...
		case Token.LC:
			...
			// MODIFIED BY AMATERAS
			scope.sourceStart = ts.sourceCursor;
			pushScope(scope);
			try {
				statements(scope);
				...
				// MODIFIED BY AMATERAS
				scope.sourceEnd = ts.sourceCursor - 1;
				return scope;
			} finally {
				popScope();
			}
	 ...
	 * 
	 * @param inFor    true if we are currently in the midst of the init clause of a
	 *                 for.
			...
			if (tt == Token.LB || tt == Token.LC) {
				...
				if (!first) {
					...
					if (s != null && s.length() > 0) {
						int offset = ts.sourceString.indexOf(s, start);
						if (offset >= 0) {
							start += offset - start;
						}
					}
	...
	void defineSymbol(int declType, boolean ignoreNotInBlock, String name) {
		...
		if (symbol != null && (symbol.declType == Token.CONST || declType == Token.CONST)) {
			...
			case Token.LP:
				...
				Node.Scope.Symbol fnSymbol = new Node.Scope.Symbol(declType, name);
				currentScriptOrFn.putSymbol(name, fnSymbol);
				fnSymbol.sourceStart = ts.sourceCursor - name.length() - 1;
				fnSymbol.jsDocNode = ts.jsDocNode;
				ts.jsDocNode = null;
				break;
					...
					symbol.declType == Token.CONST ? "msg.const.redecl"
							: symbol.declType == Token.LET ? "msg.let.redecl"
									: symbol.declType == Token.VAR ? "msg.var.redecl"
											: symbol.declType == Token.FUNCTION ? "msg.fn.redecl" : "msg.parm.redecl",
					name);
		}
	}
	...
	private Node memberExpr(boolean allowCallSyntax) throws IOException, ParserException {
		...
		if (tt == Token.NEW) {
			...
			/*
			 * XXX there's a check in the C source against "too many constructor arguments"
			 * - how many do we claim to support?
			 */
	...
	private Node arrayComprehension(String arrayName, Node expr) throws IOException, ParserException {
		...
		if (tt == Token.LB || tt == Token.LC) {
			...
			expr = nf.createBinary(Token.COMMA, nf.createAssignment(Token.ASSIGN, primaryExpr(), nf.createName(name)),
					expr);
			...
			for (;;) {
				...
				if (tt == Token.COMMA) {
					...
					try {
						...
						Node init = new Node(Token.EXPR_VOID, nf.createAssignment(Token.ASSIGN, nf.createName(tempName),
								nf.createCallOrNew(Token.NEW, nf.createName("Array"))), ts.getLineno());
		...
		case Token.LC: {
			...
			if (!matchToken(Token.RC)) {
				...
				commaloop: do {
					...
					tt = peekToken();
					// MODIFIED BY AMATERAS
					int innerStart = ts.sourceCursor;
					switch (tt) {
					case Token.NAME:
					case Token.STRING:
						consumeToken();
					...
					case Token.NUMBER:
						...
						// MODIFIED BY AMATERAS
						innerStart -= ts.stringBufferTop + 1;
						plainProperty(elems, property);
						break;
					...
					// MODIFIED BY AMATERAS
					if (elems.size() != 0) {
						((Node) elems.peek()).sourceStart = innerStart;
					}
				} while (matchToken(Token.COMMA));
...
//            return nf.createObjectLiteral(elems);
			// MODIFIED BY AMATERAS
			Node node = nf.createObjectLiteral(elems);
			node.sourceStart = start - 1;
			node.sourceEnd = ts.sourceCursor;
			return node;
		}
		...
		case Token.LP:
			...
			/*
			 * Brendan's IR-jsparse.c makes a new node tagged with TOK_LP here... I'm not
			 * sure I understand why. Isn't the grouping already implicit in the structure
			 * of the parse tree? also TOK_LP is already overloaded (I think) in the C IR as
			 * 'function call.'
			 */
			decompiler.addToken(Token.LP);
			pn = expr(false);
