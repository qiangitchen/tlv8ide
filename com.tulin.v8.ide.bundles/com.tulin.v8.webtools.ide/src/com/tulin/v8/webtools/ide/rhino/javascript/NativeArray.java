...
public class NativeArray extends IdScriptableObject {
	...
	/*
	 * Optimization possibilities and open issues: - Long vs. double schizophrenia.
	 * I suspect it might be better to use double throughout.
	 *
	 * - Functions that need a new Array call "new Array" in the current scope
	 * rather than using a hardwired constructor; "Array" could be redefined. It
	 * turns out that js calls the equivalent of "new Array" in the current scope,
	 * except that it always gets at least an object back, even when Array == null.
	 */
	...
	protected void fillConstructorProperties(IdFunctionObject ctor) {
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_join, "join", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_reverse, "reverse", 1);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_sort, "sort", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_push, "push", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_pop, "pop", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_shift, "shift", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_unshift, "unshift", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_splice, "splice", 2);
		addIdFunctionProperty(ctor, ARRAY_TAG, ConstructorId_concat, "concat", 2);
	...
	public void delete(int index) {
		if (dense != null && 0 <= index && index < dense.length && !isSealed()
				&& (denseOnly || !isGetterOrSetter(null, index, true))) {
	...
	/**
	 * Change the value of the internal flag that determines whether all storage is
	 * handed by a dense backing array rather than an associative store.
	 * 
	 * @param denseOnly new value for denseOnly flag
	 * @throws IllegalArgumentException if an attempt is made to enable denseOnly
	 *                                  after it was disabled; NativeArray code is
	 *                                  not written to handle switching back to a
	...
	private void setLength(Object val) {
		/*
		 * XXX do we satisfy this? 15.4.5.1 [[Put]](P, V): 1. Call the [[CanPut]] method
		 * of A with name P. 2. If Result(1) is false, return. ?
		 */
		...
		if (denseOnly) {
			if (longVal < length) {
				...
				length = longVal;
				return;
			} else if (longVal < MAX_PRE_GROW_SIZE && longVal < (length * GROW_FACTOR)
	...
	/*
	 * Support for generic Array-ish objects. Most of the Array functions try to be
	 * generic; anything that has a length property is assumed to be an array.
	 * getLengthProperty returns 0 if obj does not have the length property or its
	 ...
	 * avoids unnecessary object creation that would be necessary to use the general
	 * ScriptRuntime.get/setElem functions... though this is probably premature
	 * optimization.
	 */
	...
	private static String toStringHelper(Context cx, Scriptable scope, Scriptable thisObj, boolean toSource,
			boolean toLocale) {
		/*
		 * It's probably redundant to handle long lengths in this function;
		 * StringBuilders are limited to 2^31 in java.
		 */
	 ...
	 */
	private static void heapsort(Context cx, Scriptable scope, Object[] array, int length, Object cmp,
			Object[] cmpBuf) {
		if (length <= 1)
			Kit.codeBug();
	...
	/**
	 * pivot and child heaps of i should be made into heap starting at i, original
	 * array[i] is never used to have less array access during sorting.
	 */
	private static void heapify(Context cx, Scriptable scope, Object pivot, Object[] array, int i, int end, Object cmp,
			Object[] cmpBuf) {
		for (;;) {
			int child = i * 2 + 1;
			if (child >= end) {
	...
	/**
	 * Version of heapsort that call getElem/setElem on target to query/assign array
	 * elements instead of Java array access
	 */
	private static void heapsort_extended(Context cx, Scriptable scope, Scriptable target, long length, Object cmp,
			Object[] cmpBuf) {
		if (length <= 1)
			Kit.codeBug();
	...
	private static void heapify_extended(Context cx, Scriptable scope, Object pivot, Scriptable target, long i,
			long end, Object cmp, Object[] cmpBuf) {
		for (;;) {
			long child = i * 2 + 1;
			if (child >= end) {
				break;
	...
	private static Object js_push(Context cx, Scriptable thisObj, Object[] args) {
		...
		/*
		 * If JS1.2, follow Perl4 by returning the last thing pushed. Otherwise, return
	...
	private static Object js_shift(Context cx, Scriptable thisObj, Object[] args) {
		...
		if (length > 0) {
			...
			/*
			 * Slide down the array above the first element. Leave i set to point to the
			 * last element.
			 */
			if (length > 0) {
				for (i = 1; i <= length; i++) {
	...
	private static Object js_splice(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		if (count != 0) {
			if (count == 1 && (cx.getLanguageVersion() == Context.VERSION_1_2)) {
				/*
				 * JS lacks "list context", whereby in Perl one turns the single scalar that's
				 * spliced out into an array just by assigning it to @single instead of $single,
				 * or by using it as Perl push's first argument, for instance.
				 *
				 * JS1.2 emulated Perl too closely and returned a non-Array for the
				 * single-splice-out case, requiring callers to test and wrap in [] if
				 * necessary. So JS1.3, default, and other versions all return an array of
	...
	private static Scriptable js_concat(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		/*
		 * Put the target in the result array; only add it as an array if it looks like
		 * one.
		 */
		...
		/*
		 * Copy from the arguments into the result. If any argument has a numeric length
		 * property, treat it as an array and add elements separately; otherwise, just
		 * copy the argument.
		 */
		for (int i = 0; i < args.length; i++) {
	...
	private Object indexOfHelper(Context cx, Scriptable thisObj, Object[] args, boolean isLast) {
		...
		if (isLast) {
			...
			/*
			 * From
			 * http://developer.mozilla.org/en/docs/Core_JavaScript_1.5_Reference:Objects:
			 * Array:lastIndexOf The index at which to start searching backwards. Defaults
			 * to the array's length, i.e. the whole array will be searched. If the index is
			 ...
			 * From
			 * http://developer.mozilla.org/en/docs/Core_JavaScript_1.5_Reference:Objects:
			 * Array:indexOf The index at which to begin the search. Defaults to 0, i.e. the
			 * whole array will be searched. If the index is greater than or equal to the
			 * length of the array, -1 is returned, i.e. the array will not be searched. If
			 * negative, it is taken as the offset from the end of the array. Note that even
			 * when the index is negative, the array is still searched from front to back.
	...
	private Object iterativeMethod(Context cx, int id, Scriptable scope, Scriptable thisObj, Object[] args) {
		...
		for (long i = 0; i < length; i++) {
			...
			Object elem = (i > Integer.MAX_VALUE) ? ScriptableObject.getProperty(thisObj, Long.toString(i))
...
// #/generated#
	...
	private static final int Id_constructor = 1, Id_toString = 2, Id_toLocaleString = 3, Id_toSource = 4, Id_join = 5,
			...
			Id_concat = 13, Id_slice = 14, Id_indexOf = 15, Id_lastIndexOf = 16, Id_every = 17, Id_filter = 18,
			Id_forEach = 19, Id_map = 20, Id_some = 21,

			MAX_PROTOTYPE_ID = 21;

// #/string_id_map#

	private static final int ConstructorId_join = -Id_join, ConstructorId_reverse = -Id_reverse,
			ConstructorId_sort = -Id_sort, ConstructorId_push = -Id_push, ConstructorId_pop = -Id_pop,
			ConstructorId_shift = -Id_shift, ConstructorId_unshift = -Id_unshift, ConstructorId_splice = -Id_splice,
			ConstructorId_concat = -Id_concat, ConstructorId_slice = -Id_slice, ConstructorId_indexOf = -Id_indexOf,
			ConstructorId_lastIndexOf = -Id_lastIndexOf, ConstructorId_every = -Id_every,
			ConstructorId_filter = -Id_filter, ConstructorId_forEach = -Id_forEach, ConstructorId_map = -Id_map,
