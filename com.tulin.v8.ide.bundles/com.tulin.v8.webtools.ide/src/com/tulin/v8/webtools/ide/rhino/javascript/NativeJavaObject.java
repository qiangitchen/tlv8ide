 ...
 * It reflect fields directly, and uses NativeJavaMethod objects to reflect
...
public class NativeJavaObject implements Scriptable, Wrapper, Serializable {
	...
	/**
	 * Derive a ranking based on how "natural" the conversion is. The special value
	 * CONVERSION_NONE means no conversion is possible, and CONVERSION_NONTRIVIAL
	 * signals that more type conformance testing is required. Based on
	 ...
	 * @see com.tulin.v8.webtools.ide.jseditor.rhino.javascript.Context#jsToJava(Object,
	 *      Class)
	 */
	...
	private static Object coerceToNumber(Class<?> type, Object value) {
		...
		if (type == Character.TYPE || type == ScriptRuntime.CharacterClass) {
			...
			return Character.valueOf(
					(char) toInteger(value, ScriptRuntime.CharacterClass, Character.MIN_VALUE, Character.MAX_VALUE));
		}
		...
		if (type == ScriptRuntime.IntegerClass || type == Integer.TYPE) {
			if (valueClass == ScriptRuntime.IntegerClass) {
				...
				return toInteger(value, ScriptRuntime.IntegerClass, Integer.MIN_VALUE, Integer.MAX_VALUE);
			}
		}
		...
		if (type == ScriptRuntime.LongClass || type == Long.TYPE) {
			if (valueClass == ScriptRuntime.LongClass) {
				...
				/*
				 * Long values cannot be expressed exactly in doubles. We thus use the largest
				 * and smallest double value that has a value expressible as a long value. We
				 * build these numerical values from their hexidecimal representations to avoid
				 * any problems caused by attempting to parse a decimal representation.
				 */
	...
	static void reportConversionError(Object value, Class<?> type) {
		...
		throw Context.reportRuntimeError2("msg.conversion.not.allowed", String.valueOf(value),
	...
	static {
		...
		Class<?> cl = Kit.classOrNull("com.tulin.v8.webtools.ide.jseditor.rhino.javascript.JavaAdapter");
		if (cl != null) {
			try {
