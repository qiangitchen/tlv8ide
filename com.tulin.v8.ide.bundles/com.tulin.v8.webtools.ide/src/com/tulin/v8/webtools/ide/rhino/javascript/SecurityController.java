 ...
 * <p>
 * Three main pieces of functionality are required to implement security for
 * JavaScript. First, it must be possible to define classes with an associated
 * security domain. (This security domain may be any object incorporating notion
 * of access restrictions that has meaning to an embedding; for a client-side
 * JavaScript embedding this would typically be java.security.ProtectionDomain
 * or similar object depending on an origin URL and/or a digital certificate.)
 * Next it must be possible to get a security domain object that allows a
 * particular action only if all security domains associated with code on the
 * current Java stack allows it. And finally, it must be possible to execute
 * script code with associated security domain injected into Java stack.
 * <p>
 * These three pieces of functionality are encapsulated in the
 * SecurityController class.
 *
 * @see com.tulin.v8.webtools.ide.jseditor.rhino.javascript.Context#setSecurityController(SecurityController)
...
// The method must NOT be public or protected
	 ...
	 * operations. The global controller takes precedence over already installed
	 * {@link Context}-specific controllers and cause any subsequent call to
	 * {@link Context#setSecurityController(SecurityController)} to throw an
	 * exception.
	 * <p>
	 ...
	 * 
	 * @param parentLoader   parent class loader to delegate search for classes not
	 *                       defined by the class loader itself
	 * @param securityDomain some object specifying the security context of the code
	 *                       that is defined by the returned class loader.
	 */
	public abstract GeneratedClassLoader createClassLoader(ClassLoader parentLoader, Object securityDomain);

	/**
	 * Create {@link GeneratedClassLoader} with restrictions imposed by staticDomain
	 * and all current stack frames. The method uses the SecurityController instance
	 * associated with the current {@link Context} to construct proper dynamic
	 * domain and create corresponding class loader. <par> If no SecurityController
	 * is associated with the current {@link Context} , the method calls
	 ...
	 * the <i>execWithDomain</i> call and <i>securityDomain</i>. Any call to
	 * {@link #getDynamicSecurityDomain(Object)} during execution of
	 * <tt>callable.call(cx, scope, thisObj, args)</tt> should return a domain
	 * incorporate restrictions imposed by <i>securityDomain</i> and Java stack on
	 * the moment of callWithDomain invocation.
	 * <p>
