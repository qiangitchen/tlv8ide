...
class Base64 {
	...
	public static void decode(String s, OutputStream out) throws IOException {
		...
		for (int i = 0; i < count; i++) {
			...
			case 0:
				n = i / 3 * 4;
				b = (byte) (fromChar(s.charAt(n)) << 2 | fromChar(s.charAt(n + 1)) >> 4);
				out.write(b);
				break;
			case 1:
				n = i / 3 * 4 + 1;
				b = (byte) ((fromChar(s.charAt(n)) & 0xF) << 4 | fromChar(s.charAt(n + 1)) >> 2);
				out.write(b);
				break;
			case 2:
				n = i / 3 * 4 + 2;
				b = (byte) ((fromChar(s.charAt(n)) & 0x3) << 6 | fromChar(s.charAt(n + 1)));
				out.write(b);
			}
		}
	}
