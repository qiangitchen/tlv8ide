...
function addHoverDom(treeId, treeNode) {
	...
	if (btn)
		btn.bind("click", function() {
			...
			var r = justep.yn.XMLHttpRequest("insertflwFolderAction", param,
					"post", true, null);
...
function afterEditName(event, treeId, treeNode) {
	...
	var r = justep.yn.XMLHttpRequest("editflwFolderAction", param, "post",
			true, null);
}
...
function onRemove(e, treeId, treeNode) {
	...
	param.set("sprocessid", treeNode.SPROCESSID);
	var r = justep.yn.XMLHttpRequest("deleteflwFolderAction", param, "post",
			true, function() {
				clear_draw_board();
			});
}
...
function initBody() {
	...
	var labelid = "No,SPROCESSNAME,SPROCESSID,FENABLED,SPROCESSACTY";
	var labels = "No.,流程名称,流程ID,状态,流程环节";
	var datatype = "ro,string,ro,html:getEnabledStatus,ro";
	var labelwidth = "40,120,240,40,0";
	var dataAction = {
		"queryAction" : "getGridAction",
		"savAction" : "saveAction",
		"deleteAction" : "deleteAction"
	};
	var maingrid = new justep.yn.createGrid(det, labelid, labels, labelwidth,
	...
	var spsncode = justep.yn.Context.getCurrentPersonCode();
	if (spsncode == "SYSTEM") {
		maingrid.grid.insertSelfBar("生效/失效", "64px",
				"setEnabledStatus(this,1)",
				"/JBIZ/comon/image/toolbar/enable.png");
		maingrid.grid
				.insertSelfBar("", "5px", "",
						"/JBIZ/comon/image/toolbar/standard_toolbar/standard/compart.gif");
...
function setEnabledStatus(obj, status) {
	if (status == 1) {
		obj.src = "/JBIZ/comon/image/toolbar/disable.png";
		...
		obj.src = "/JBIZ/comon/image/toolbar/enable.png";
		var actGrid = document.getElementById("processListGrid_div").grid;
...
function dailogEngin(param) {
	...
	var jsonStr = cGrid.getValueByName("SPROCESSACTY", cGrid.CurrentRowId);
	return {
		id : id,
		name : name,
		jsonStr : jsonStr
