...
function Node() {
	...
	this.init = function() {
		...
		if (SVG.supported) {
			...
			obj = document.createElement('v:rect');
			obj.strokecolor = 'blue';
			obj.strokeweight = '1';
		}
		groupObj.appendChild(obj);
		...
		if (this.shadowFlag) {
			if (SVG.supported) {
				...
				$(obj).css("-moz-box-shadow", "5px 5px 5px #b3b3b3");
				$(obj).css("-webkit-box-shadow", "5px 5px 5px #b3b3b3");
				$(obj).css("box-shadow", "5px 5px 5px #b3b3b3");
			} else {
		...
		if (this.textFlag) {
			...
			if (SVG.supported) {
				...
				textObj.style.marginTop = '6px';
			} else {
				textObj = document.createElement('v:textbox');
				textObj.inset = '2pt,5pt,2pt,5pt';
			}
			textObj.id = this.id + "_Label";
			textObj.style.textAlign = 'center';
			textObj.style.color = '#000000';
			textObj.style.fontSize = '10pt';
			textObj.style.lineHeight = '25px';
			textObj.innerHTML = this.name;
			obj.appendChild(textObj);
			this.textObj = textObj;
		}
	...
	this.setSelected = function() {
		if (this.shadowFlag) {
			if (SVG.supported) {
				$(this.obj).css("-moz-box-shadow", "5px 5px 5px green");
				$(this.obj).css("-webkit-box-shadow", "5px 5px 5px green");
				$(this.obj).css("box-shadow", "5px 5px 5px green");
			} else {
				this.shadowObj.color = 'green';
			}
		}
		if (SVG.supported) {
			$(this.obj).css("border-color", "green");
		} else {
			this.obj.strokecolor = 'green';
		}
	...
	this.clearSelected = function() {
		if (this.shadowFlag) {
			if (SVG.supported) {
				$(this.obj).css("-moz-box-shadow", "5px 5px 5px #b3b3b3");
				$(this.obj).css("-webkit-box-shadow", "5px 5px 5px #b3b3b3");
				$(this.obj).css("box-shadow", "5px 5px 5px #b3b3b3");
			} else {
				this.shadowObj.color = '#b3b3b3';
			}
		}
		if (SVG.supported) {
			$(this.obj).css("border-color", "blue");
		} else {
			this.obj.strokecolor = 'blue';
		}
	...
	this.setPassOn = function() {// 当前
		if (this.shadowFlag) {
			if (SVG.supported) {
				$(this.obj).css("-moz-box-shadow", "5px 5px 5px blue");
				$(this.obj).css("-webkit-box-shadow", "5px 5px 5px blue");
				$(this.obj).css("box-shadow", "5px 5px 5px blue");
			} else {
...
function Condition() {
	...
	this.init = function() {
		...
		if (SVG.supported) {
			obj = document.createElement('div');
			obj.id = this.id;
			$(obj).appendTo($(groupObj));
			var draw = SVG(obj.id);
			var shape = draw.polyline("0,25 60,50 120,25 60,0 0,25").stroke({
				width : 1,
				color : 'blue'
			}).fill("#fff");
			obj.shape = shape;
		...
		if (this.textFlag) {
			...
			if (SVG.supported) {
				textObj = document.createElement('div');
				textObj.style.position = 'inherit';
				textObj.style.left = '2px';
				textObj.style.top = '15px';
				textObj.style.zIndex = 1;
				textObj.style.width = '100%';
				textObj.style.height = '100%';
			} else {
				textObj = document.createElement('v:textbox');
				textObj.inset = '2pt,9pt,2pt,9pt';
			}
			textObj.style.textAlign = 'center';
			textObj.style.color = 'blue';
			textObj.style.fontSize = '9pt';
...
function NodeOval() {
	...
	this.init = function() {
		...
		if (SVG.supported) {
			...
			$(obj).appendTo($(groupObj));
			var draw = SVG(obj.id);
			var circle = draw.circle(this.width);
			if (this.type == 'start') {
				circle.stroke({
					width : 1,
					color : '#33CC00'
	...
	this.setSelected = function() {
		this.shadowObj.on = 'T';
		if (SVG.supported) {
			$(this.obj).css("border", "1px dashed green");
		}
		if (this.textObj)
			this.textObj.style.color = 'green';
		this.selected = true;
...
function PolyLine() {
	...
	this.init = function() {
		...
		if (SVG.supported) {
			...
			var draw = SVG(obj.id);
			var pline = draw.polyline(this.getPointsValue()).stroke({
				width : 1,
				color : 'blue',
				filled : false
			}).fill('none');
		...
		this.obj = obj;
		if (SVG.supported) {
			obj.pline.attr("marker-end", "url(#idArrow)");
		} else {
			var strokeObj = document.createElement('v:stroke');
			obj.appendChild(strokeObj);
			strokeObj.endArrow = "Classic";
		...
		if (this.textFlag && (this.name.indexOf("line_") < 0)
			...
			if (SVG.supported) {
				...
				textObj.style.color = 'blue';
				textObj.style.fontSize = '9pt';
				textObj.style.left = this.fromX
						+ Math.abs(this.toX - this.fromX) / 2;
				textObj.style.top = this.fromY
						+ Math.abs(this.toY - this.fromY) / 2 - 35;
				textObj.innerHTML = this.name;
				obj.appendChild(textObj);
				this.textObj = textObj;
			} else {
	...
	this.setFrom = function(x, y, obj) {
		...
		this.polyDot = [];
		if (SVG.supported) {
			this.obj.pline.plot(this.getPointsValue());
		} else {
	...
	this.getPointsValue = function() {
		var res;
		var svgpad = 8; // 校正8为箭头像素
		var margin = 30;// margin 30矫正
		if (SVG.supported) {
			var lx = this.fromX, ly = this.fromY;
			res = this.fromX + ',' + (this.fromY - margin);
			for (var i = 0; i < this.polyDot.length; i++) {
				res += ' ' + this.polyDot[i].x + ','
						+ (this.polyDot[i].y - margin);
				lx = this.polyDot[i].x;
				ly = this.polyDot[i].y;
			}
			var chX = (this.toX > lx) ? -svgpad : svgpad;
			var chY = (this.toY > ly) ? -svgpad : svgpad;
			if (Math.abs(this.toX - lx) < Math.abs(svgpad)) {
				chX = 0;
			}
			if (Math.abs(this.toY - ly) < Math.abs(svgpad)) {
				chY = 0;
			}
			res += ' ' + (this.toX + chX) + ',' + (this.toY + chY - margin);
		} else {
			res = this.fromX + ',' + this.fromY;
			for (var i = 0; i < this.polyDot.length; i++) {
	...
	this.moveEnd = function(x, y, mouseX, mouseY) {
		if (SVG.supported) {
			this.obj.style.cursor = 'band';
		} else {
			this.obj.parentNode.style.cursor = 'band';
		}
		var point;
		if (SVG.supported) {
			point = this.obj.pline.attr("points");
		} else {
			point = this.obj.points.value;
		}
