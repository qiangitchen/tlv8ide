package com.tulin.v8.flowdesigner.server.common;
...
public class FlowWebServer {
	...
	public FlowWebServer() {
		this(0);
	}

	public FlowWebServer(int port) {
		this.port = port;
	}
	...
	public static WebServerContent getWebServerContent(HTTPRequest httpRequest) {
		...
		if (index != -1) {
			...
			if ("class".equals(type)) {
				...
				FlowWebServer webServer = (FlowWebServer) ObjectRegistry.getInstance()
			...
			if ("classpath".equals(type)) {
				index = parameter.indexOf('/');
				final FlowWebServer webServer = (FlowWebServer) ObjectRegistry.getInstance()
						.get(Integer.parseInt(parameter.substring(0, index)));
				if (webServer == null) {
					return null;
				}
				...
				return new WebServerContent() {
					...
					public InputStream getInputStream() {
						try {
							...
							return FlowWebServer.class.getResourceAsStream('/' + resourcePath);
						} catch (Exception e) {
							e.printStackTrace();
							return null;
						}
	...
	private static FlowWebServer webServer;
	...
	static {
		...
		FlowWebServer.hostAddress = hostAddress;
	}
	...
	public static FlowWebServer getDefaultWebServer() {
		synchronized (LOCK) {
			if (webServer != null) {
				return webServer;
			}
			webServer = new FlowWebServer();
			try {
