		...
		try {
			...
			while (conIt.hasNext()) {
				...
				section.setAttributeValue("name", name);
				{
					...
					// 达梦数据库 除了SYSDBA用户只显示自己用户名对应的schema
//					if (DBUtils.IsDMDB(name) && !"SYSDBA".equals(schema)) {
//						java.sql.Connection con;
//						try {
//							con = DBUtils.getAppConn(name);
//							String[] schemas = SchemaSearcher.execute(con);
//							for (String sname : schemas) {
//								if (!sname.equals(schema)) {
//									Element item18_11 = item18.addElement("schema");
//									item18_11.setAttributeValue("checked", "false");
//									item18_11.setAttributeValue("name", sname);
//								}
//							}
//						} catch (Exception e) {
//							e.printStackTrace();
