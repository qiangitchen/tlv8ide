...
public class DB2SQLCreatorFactoryTest extends OracleSQLCreatorFactoryTest {
	...
	public void testCreateAddColumnDDL() {
		...
		assertEquals("ALTER TABLE SCOTT.EMP ADD DEPTNO NUMBER(1)", sqls[0]);
		
		
		newCol = createColumn("DEPTNO", "NUMBER", "123", "コメント");
		newCol.setSize("1");
		sqls = f.createAddColumnDDL(newCol);
		assertEquals(1, sqls.length);
		assertEquals("ALTER TABLE SCOTT.EMP ADD DEPTNO NUMBER(1) DEFAULT 123", sqls[0]);
		...
		sqls = f.createAddColumnDDL(newCol);
		assertEquals(1, sqls.length);
		assertEquals("ALTER TABLE \"SCOTT-1\".\"EMP-1\" ADD \"DEPTNO-1\" NUMBER(1)", sqls[0]);
