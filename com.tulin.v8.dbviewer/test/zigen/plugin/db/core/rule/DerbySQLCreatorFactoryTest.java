...
public class DerbySQLCreatorFactoryTest extends OracleSQLCreatorFactoryTest {
	...
	public void testCreateRenameTableDDL() {
		...
		assertEquals("RENAME TABLE SCOTT.EMP TO HOGE", sql);
		
		table = createTable("SCOTT-1", "EMP-1");
		f = AbstractSQLCreatorFactory.getFactory(table.getDbConfig(), table);
		f.setVisibleSchemaName(true);
		sql = f.createRenameTableDDL("H'OGE-1");
		assertEquals("RENAME TABLE \"SCOTT-1\".\"EMP-1\" TO \"H'OGE-1\"", sql);
	}
	...
	public void testCreateAddColumnDDL() {
		...
		assertEquals("ALTER TABLE SCOTT.EMP ADD COLUMN DEPTNO CHAR", sqls[0]);
		assertEquals("", sqls[1]);
		...
		assertEquals(2, sqls.length);
		assertEquals("ALTER TABLE SCOTT.EMP ADD COLUMN DEPTNO VARCHAR(1) DEFAULT 123", sqls[0]);
		assertEquals("", sqls[1]);
		...
		sqls = f.createAddColumnDDL(newCol);
		assertEquals(2, sqls.length);
		assertEquals("ALTER TABLE \"SCOTT-1\".\"EMP-1\" ADD COLUMN \"DEPTNO-1\" VARCHAR(1)", sqls[0]);
		assertEquals("", sqls[1]);
	...
	public void testCreateModifyColumnDDL() {
		...
		assertEquals("", sqls[0]);
		assertEquals("ALTER TABLE SCOTT.EMP ALTER COLUMN EMONO DEFAULT abc", sqls[1]);
		assertEquals("ALTER TABLE SCOTT.EMP ALTER COLUMN EMONO NOT NULL", sqls[2]);
		...
		assertEquals(3, sqls.length);
		assertEquals("", sqls[0]);
		assertEquals("ALTER TABLE \"SCOTT-1\".\"EMP-1\" ALTER COLUMN \"EMONO-1\" DEFAULT abc", sqls[1]);
		assertEquals("ALTER TABLE \"SCOTT-1\".\"EMP-1\" ALTER COLUMN \"EMONO-1\" NOT NULL", sqls[2]);
