package com.tulin.v8.ide.wizards.gridSelectDialog;
...
public class SelectListPermision extends WizardPage {
	...
	public SelectListPermision() {
		super("selectListPermision");
	}
		...
		ptable.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				if (Messages.getString("wizardsaction.dataselect.message.dataperm").equals(name)) {
					setTitle("列表单选对话框-主数据");
					stackLayout.topControl = dataComp;
				}
				if ("列表配置".equals(name)) {
					setTitle("列表单选对话框-列表配置");
					stackLayout.topControl = confComp;
				}
				if ("对话框返回信息配置".equals(name)) {
					setTitle("列表单选对话框-对话框返回信息配置");
	...
	private Composite setTablepermision(Composite shell) {
		...
		tablegrid = new Table(composite, SWT.BORDER | SWT.V_SCROLL
				| SWT.FULL_SELECTION);
					...
					if (IDialogConstants.OK_ID == result) {
						...
						if (cells != null && !"".equals(cells)) {
							...
							for (int i = 0; i < cellarray.length; i++) {
								setColums(cellarray[i], tablegrid, dataType,
										label, true);
								setDeColums(cellarray[i], detablegrid,
										dataType, label, true);
							}
						}
						setPageCompleteEnable();
		...
		adddel.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				boolean result = MessageDialog.openConfirm(getShell(), Messages.getString("wizardsaction.dataselect.message.clmremoveConfirm"),
						Messages.getString("wizardsaction.dataselect.message.clmremoveConfirmmsg"));
				if (result) {
					String cellid = tablegrid.getItem(
							tablegrid.getSelectionIndex()).getText();
					setColums(cellid, tablegrid, null, null, false);
					setDeColums(cellid, detablegrid, null, null, false);
					setPageCompleteEnable();
				}
			}
		});
	...
	private void setColums(String id, Table tablegrid,
			Map<String, String> datatype, Map<String, String> label,
			boolean isadd) {
		...
		if (isadd) {
			if (chIdIndex < 0) {
				String dataType = DataType.getDataTypeBydatabase(datatype.get(
						id).toUpperCase());
				TableItem tableitem = new TableItem(tablegrid, SWT.NONE);
				tableitem.setText(new String[] { id, label.get(id), "80",
						dataType });
				columns.add(id);
	...
	private Composite setdeTablepermision(Composite shell) {
		...
		Label label = new Label(composite, SWT.FLAT | SWT.WRAP);
		label.setText("下表中勾选需要返回的列");

		detablegrid = new Table(composite, SWT.BORDER | SWT.V_SCROLL
		...
		detablegrid.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				TableItem item = (TableItem) e.item;
				if (item.getChecked()) {
					decolumns.add(item.getText(0));
				} else {
					decolumns.remove(item.getText(0));
				}
			}
		});
	...
	private void setDeColums(String id, Table tablegrid,
			Map<String, String> datatype, Map<String, String> label,
			boolean isadd) {
		TableItem[] tbItems = tablegrid.getItems();
		int chIdIndex = -1;
		...
		if (isadd) {
			if (chIdIndex < 0) {
				String dataType = DataType.getDataTypeBydatabase(datatype.get(
						id).toUpperCase());
				TableItem tableitem = new TableItem(tablegrid, SWT.NONE);
				tableitem.setText(new String[] { id, label.get(id), dataType });
			}
		} else {
			if (chIdIndex > -1) {
				tablegrid.remove(chIdIndex);
	...
	private void setPageCompleteEnable() {
		boolean enable = (dbkey != null & table != null
				&& tablegrid.getItems().length > 0 && detablegrid.getItems().length > 0);
		setPageComplete(enable);
	}

	@Override
	public IWizardPage getNextPage() {
		return getWizard().getPage("selectListPageEnd");
	}
