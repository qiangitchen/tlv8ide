...
import com.tulin.v8.ide.views.navigator.dialog.RenameDialog;
...
public class RenameAction extends Action implements Runnable {
	...
	public void run() {
		...
		if (obj != null) {
			...
			RenameDialog dialog = new RenameDialog(StudioPlugin.getShell(), node);
	...
	void ensure(TreeNode node) {
		if (newname == null || "".equals(newname)) {
			MessageDialog.openError(StudioPlugin.getShell(), Messages.getString("View.Action.Rename.3"),
	...
	void renameFile(TreeNode node, String name) {
		try {
			...
			if (filer.exists()) {
				MessageDialog.openError(viewer.getControl().getShell(), Messages.getString("View.Action.Rename.3"),
						Messages.getString("View.Action.Rename.8") + name + Messages.getString("View.Action.Rename.9"));
				return;
			}
			file.renameTo(filer);
			node.setName(name);
