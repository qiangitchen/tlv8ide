...
public class EditorExecutionContext implements IExecutionDelegate {
	...
	/**
	 * Reusable runnable for the Display execution queue to cut down on garbage
	 * creation. Will make use of the progress service if possible.
	 */
	private static class ReusableUIRunner implements Runnable,
			IRunnableWithProgress {
		private XMLEditor editor;
		private ISafeRunnable fRunnable = null;

		ReusableUIRunner(XMLEditor part) {
			super();
			editor = part;
		}
		...
		public void run() {
			...
			final IWorkbenchWindow workbenchWindow = (site == null) ? null
					: site.getWorkbenchWindow();
			final IWorkbenchSiteProgressService jobService = (IWorkbenchSiteProgressService) ((site == null) ? null
					: site.getAdapter(IWorkbenchSiteProgressService.class));
			/*
			...
			if (workbenchWindow != null && jobService != null) {
				...
				try {
					jobService.runInUI(
							workbenchWindow,
							this,
							(ISchedulingRule) editor.getEditorPart()
									.getEditorInput()
				 ...
				 * background-update mode). NOTE: this execute method itself is
				 * always called from inside of an ILock block, so another block
				 * is not not needed here for all these sycnExec's.
				 */
	...
	public EditorExecutionContext(XMLEditor editor) {
		super();
		fEditor = editor;
