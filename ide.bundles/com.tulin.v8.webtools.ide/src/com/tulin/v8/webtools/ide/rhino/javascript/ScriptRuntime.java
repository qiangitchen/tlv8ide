...
import com.tulin.v8.webtools.ide.rhino.javascript.xml.XMLObject;
...
public class ScriptRuntime {
	...
	/*
	 * There's such a huge space (and some time) waste for the Foo.class syntax: the
	 * compiler sticks in a test of a static field in the enclosing class for null
	 * and the code for creating the class value. It has to do this since the
	 * reference has to get pushed off until execution time (i.e. can't force an
	 * early load), but for the 'standard' classes - especially those in java.lang,
	 * we can trust that they won't cause problems by being loaded early.
	 */

	public final static Class<?> BooleanClass = Kit.classOrNull("java.lang.Boolean"),
			ByteClass = Kit.classOrNull("java.lang.Byte"), CharacterClass = Kit.classOrNull("java.lang.Character"),
			ClassClass = Kit.classOrNull("java.lang.Class"), DoubleClass = Kit.classOrNull("java.lang.Double"),
			FloatClass = Kit.classOrNull("java.lang.Float"), IntegerClass = Kit.classOrNull("java.lang.Integer"),
			LongClass = Kit.classOrNull("java.lang.Long"), NumberClass = Kit.classOrNull("java.lang.Number"),
			ObjectClass = Kit.classOrNull("java.lang.Object"), ShortClass = Kit.classOrNull("java.lang.Short"),
			StringClass = Kit.classOrNull("java.lang.String"), DateClass = Kit.classOrNull("java.util.Date");

	public final static Class<?> ContextClass = Kit.classOrNull("com.tulin.v8.webtools.ide.rhino.javascript.Context"),
			ContextFactoryClass = Kit.classOrNull("com.tulin.v8.webtools.ide.rhino.javascript.ContextFactory"),
			FunctionClass = Kit.classOrNull("com.tulin.v8.webtools.ide.rhino.javascript.Function"),
			ScriptableObjectClass = Kit.classOrNull("com.tulin.v8.webtools.ide.rhino.javascript.ScriptableObject");
	public static final Class<Scriptable> ScriptableClass = Scriptable.class;

	private static final String[] lazilyNames = { "RegExp",
			"com.tulin.v8.webtools.ide.rhino.javascript.regexp.NativeRegExp", "Packages",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "java",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "javax",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "org",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "com",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "edu",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "net",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "getClass",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeJavaTopPackage", "JavaAdapter",
			"com.tulin.v8.webtools.ide.rhino.javascript.JavaAdapter", "JavaImporter",
			"com.tulin.v8.webtools.ide.rhino.javascript.ImporterTopLevel", "Continuation",
			"com.tulin.v8.webtools.ide.rhino.javascript.NativeContinuation",
			// TODO Grotesque hack using literal string (xml) just to minimize
			// changes for now
			"XML", "(xml)", "XMLList", "(xml)", "Namespace", "(xml)", "QName", "(xml)", };
	...
	public static boolean isRhinoRuntimeType(Class<?> cl) {
		if (cl.isPrimitive()) {
			...
			return (cl == StringClass || cl == BooleanClass || NumberClass.isAssignableFrom(cl)
	...
	static double stringToNumber(String s, int start, int radix) {
		...
		if (sum >= 9007199254740992.0) {
			if (radix == 10) {
				 ...
				 * result from the repeated multiply-add above may be inaccurate. Call Java to
				...
				/*
				 * The number may also be inaccurate for one of these bases. This happens if the
				 * addition in value*radix + digit causes a round-down to an even least
				 * significant mantissa bit when the first dropped bit is a one. If any of the
				 * following digits in the number (which haven't been added in yet) are nonzero
				 * then the correct action would have been to round up instead of down. An
				 * example of this occurs when reading the number 0x1000000000000081, which
	...
	/**
	 * Helper function for builtin objects that use the varargs form. ECMA function
	 * formal arguments are undefined if not supplied; this function pads the
	 * argument array out to the expected length, if necessary.
	 */
	...
	/**
	 * For escaping strings printed by object and array literals; not quite the same
	 * as 'escape.'
	 */
	...
	/**
	 * Warning: this doesn't allow to resolve primitive prototype properly when many
	 * top scopes are involved
	 */
	 ...
	 */
	public static Scriptable toObjectOrNull(Context cx, Object obj, final Scriptable scope) {
		if (obj instanceof Scriptable) {
			return (Scriptable) obj;
		} else if (obj != null && obj != Undefined.instance) {
	...
	public static Scriptable toObject(Context cx, Scriptable scope, Object val) {
		...
		String className = val instanceof String ? "String"
				: val instanceof Number ? "Number" : val instanceof Boolean ? "Boolean" : null;
		if (className != null) {
			Object[] args = { val };
	...
	/**
	 * Return -1L if str is not an index or the index value as lower 32 bits of the
	 * result.
	 */
	private static long indexFromString(String str) {
		...
		if (len > 0) {
			...
			if (0 <= c && c <= 9 && len <= (negate ? MAX_VALUE_LENGTH + 1 : MAX_VALUE_LENGTH)) {
				...
				// have overflowed.
				if (i == len && (oldIndex > (Integer.MIN_VALUE / 10) || (oldIndex == (Integer.MIN_VALUE / 10)
						&& c <= (negate ? -(Integer.MIN_VALUE % 10) : (Integer.MAX_VALUE % 10))))) {
	...
	/**
	 * If str is a decimal presentation of Uint32 value, return it as long. Othewise
	 * return -1L;
	 */
	...
	/**
	 * If s represents index, then return index value wrapped as Integer and
	 * othewise return s.
	 */
	...
	/**
	 * If d is exact int value, return its value wrapped as Integer and othewise
	 ...
	 * In this case return null and make the index available as
	 * ScriptRuntime.lastIndexResult(cx). Otherwise return toString(id).
	 */
	 ...
	 * @param scope the scope that should be used to resolve primitive prototype
	 */
	public static Object getObjectProp(Object obj, String property, Context cx, final Scriptable scope) {
		Scriptable sobj = toObjectOrNull(cx, obj, scope);
		if (sobj == null) {
			throw undefReadError(obj, property);
		}
	 ...
	 * See ECMA 11.4.1
	 *
	 * In ECMA 0.19, the description of the delete operator (11.4.1) assumes that
	 * the [[Delete]] method returns a value. However, the definition of the
	 * [[Delete]] operator (8.6.2.5) does not define a return value. Here we assume
	 * that the [[Delete]] method doesn't return a value.
	 */
	...
	private static Object nameOrFunction(Context cx, Scriptable scope, Scriptable parentScope, String name,
	 ...
	 *
	 * The order of evaluation of an assignment expression involves evaluating the
	 * lhs to a reference, evaluating the rhs, and then modifying the reference with
	 * the rhs value. This method is used to 'bind' the given name to an object
	 * containing that property so that the side effects of evaluating the rhs do
	 * not affect which property is modified. Typically used in conjunction with
	 * setName.
	 *
	 * See ECMA 10.1.4
	 */
	 ...
	 * See ECMA 12.6.3.
	 *
	 * IdEnumeration maintains a ObjToIntMap to make sure a given id is enumerated
	 * only once across multiple objects in a prototype chain.
	 *
	 * XXX - ECMA delete doesn't hide properties in the prototype, but js/ref does.
	 * This means that the js/ref for..in can avoid maintaining a hash table and
	 * instead perform lookups to see if a given property has already been
	 * enumerated.
	 *
	 */
	private static class IdEnumeration implements Serializable {
		...
		Object currentId;
		int enumType; /*
						 * one of ENUM_INIT_KEYS, ENUM_INIT_VALUES, ENUM_INIT_ARRAY
						 */
	...
	public static Object enumInit(Object value, Context cx, int enumType) {
		...
		x.enumType = enumType;
		x.iterator = null;
		if (enumType != ENUMERATE_KEYS_NO_ITERATOR && enumType != ENUMERATE_VALUES_NO_ITERATOR
		...
		for (;;) {
			...
			if (id instanceof String) {
				...
				if (!x.obj.has(intId, x.obj))
					continue; // must have been deleted
				x.currentId = x.enumNumbers ? (Object) intId : String.valueOf(intId);
			}
			return Boolean.TRUE;
		}
	}
	...
	/**
	 * Prepare for calling name(...): return function corresponding to name and make
	 * current top scope available as ScriptRuntime.lastStoredScriptable() for
	 * consumption as thisObj. The caller must call
	...
	/**
	 * Prepare for calling obj[id](...): return function corresponding to obj[id]
	 * and make obj properly converted to Scriptable available as
	 * ScriptRuntime.lastStoredScriptable() for consumption as thisObj. The caller
	 * must call ScriptRuntime.lastStoredScriptable() immediately after calling this
	 * method.
	 */
	 ...
	 * ScriptRuntime.lastStoredScriptable() for consumption as thisObj. The caller
	 * must call ScriptRuntime.lastStoredScriptable() immediately after calling this
	 * method. Warning: this doesn't allow to resolve primitive prototype properly
	 * when many top scopes are involved.
	 */
	 ...
	 * method.
	 */
	public static Callable getPropFunctionAndThis(Object obj, String property, Context cx, final Scriptable scope) {
	...
	/**
	 * Perform function call in reference context. Should always return value that
	 * can be passed to {@link #refGet(Ref, Context)} or
	 * {@link #refSet(Ref, Object, Context)} arbitrary number of times. The args
	 ...
	 * args.clone(), not args array itself.
	 */
	 ...
	 * See ECMA 15.1.2.1
	 */
	public static Object evalSpecial(Context cx, Scriptable scope, Object thisArg, Object[] args, String filename,
			int lineNumber) {
		if (args.length < 1)
			return Undefined.instance;
		Object x = args[0];
	...
	public static String typeof(Object value) {
		...
		if (value == Undefined.instance)
			return "undefined";
		if (value instanceof Scriptable) {
			if (value instanceof ScriptableObject && ((ScriptableObject) value).avoidObjectDetection()) {
				return "undefined";
			}
	...
	private static Object doScriptableIncrDecr(Scriptable target, String id, Scriptable protoChainStart, Object value,
			int incrDecrMask) {
			...
			if (y instanceof Scriptable) {
				...
				if (x instanceof Wrapper && y instanceof Wrapper) {
					...
					return unwrappedX == unwrappedY
							|| (isPrimitive(unwrappedX) && isPrimitive(unwrappedY) && eq(unwrappedX, unwrappedY));
				}
				return false;
	 ...
	 * The in operator.
	 *
	 * This is a new JS 1.3 language feature. The in operator mirrors the operation
	 * of the for .. in construct, and tests whether the rhs has the property given
	 * by the lhs. It is different from the for .. in construct in that: <BR>
	...
	public static ScriptableObject getGlobal(Context cx) {
		final String GLOBAL_CLASS = "com.tulin.v8.webtools.ide.rhino.javascript.tools.shell.Global";
	...
	/**
	 * Return <tt>possibleDynamicScope</tt> if <tt>staticTopScope</tt> is present on
	 * its prototype chain and return <tt>staticTopScope</tt> otherwise. Should only
	 * be called when <tt>staticTopScope</tt> is top scope.
	 */
	...
	public static void initScript(NativeFunction funObj, Scriptable thisObj, Context cx, Scriptable scope,
			boolean evalScript) {
		if (cx.topCallScope == null)
			throw new IllegalStateException();
				...
				if (!ScriptableObject.hasProperty(scope, name)) {
					if (!evalScript) {
						...
						if (isConst)
							...
							ScriptableObject.defineProperty(varScope, name, Undefined.instance,
	...
	public static Scriptable newCatchScope(Throwable t, Scriptable lastCatchScope, String exceptionName, Context cx,
			Scriptable scope) {
		Object obj;
		boolean cacheObj;
		...
		getObj: if (t instanceof JavaScriptException) {
			...
			if (javaException != null && isVisible(cx, javaException)) {
				...
				ScriptableObject.defineProperty(errorObject, "javaException", wrap,
						ScriptableObject.PERMANENT | ScriptableObject.READONLY);
			}
			if (isVisible(cx, re)) {
				Object wrap = cx.getWrapFactory().wrap(cx, scope, re, null);
				ScriptableObject.defineProperty(errorObject, "rhinoException", wrap,
						ScriptableObject.PERMANENT | ScriptableObject.READONLY);
			}
			obj = errorObject;
		}
		...
		if (isVisible(cx, t)) {
			...
			catchScopeObject.defineProperty("__exception__", Context.javaToJS(t, scope),
					ScriptableObject.PERMANENT | ScriptableObject.DONTENUM);
		}
	...
	public static void initFunction(Context cx, Scriptable scope, NativeFunction function, int type,
	...
	public static Scriptable newArrayLiteral(Object[] objects, int[] skipIndices, Context cx, Scriptable scope) {
		...
		Scriptable arrayObj = cx.newObject(scope, "Array", ScriptRuntime.emptyArgs);
		int skip = 0;
		for (int i = 0, j = 0; i != length; ++i) {
			if (skip != skipCount && skipIndices[skip] == i) {
				++skip;
	...
	/**
	 * This method is here for backward compat with existing compiled code. It is
	 * called when an object literal is compiled. The next instance will be the
	 * version called from new code.
	 * 
	 * @deprecated This method only present for compatibility.
	 */
	public static Scriptable newObjectLiteral(Object[] propertyIds, Object[] propertyValues, Context cx,
			Scriptable scope) {
	...
	public static Scriptable newObjectLiteral(Object[] propertyIds, Object[] propertyValues, int[] getterSetters,
	...
	/**
	 * This is an interface defining a message provider. Create your own
	 * implementation to override the default error message provider.
	 *
	 * @author Mike Harm
	 */
	public interface MessageProvider {
		...
		/**
		 * Returns a textual message identified by the given messageId, parameterized by
		 * the given arguments.
		 *
	...
	/*
	 * OPT there's a noticable delay for the first error! Maybe it'd make sense to
	 * use a ListResourceBundle instead of a properties file to avoid (synchronized)
	 * text parsing.
	 */
	private static class DefaultMessageProvider implements MessageProvider {
		public String getMessage(String messageId, Object[] arguments) {
			final String defaultResource = "com.tulin.v8.webtools.ide.rhino.javascript.resources.Messages";
			...
			/*
			 * It's OK to format the string, even if 'arguments' is null; we need to format
			 * it anyway, to make double ''s collapse to single 's.
			 */
	...
	public static EcmaError constructError(String error, String message, String sourceName, int lineNumber,
	...
	private static void warnAboutNonJSObject(Object nonJSObject) {
		String message = "RHINO USAGE WARNING: Missed Context.javaToJS() conversion:\n"
				+ "Rhino runtime detected object " + nonJSObject + " of class " + nonJSObject.getClass().getName()
