...
//		enableAutoActivateCompletionOnType(true);
	...
	private void updateProcessorToken(IContentAssistProcessor processor, IDocument document) {
		...
		try {
			Method m = this.getClass().getMethod("removeContentAssistProcessor", IContentAssistProcessor.class);
			if (m != null) {
		...
		try {
			...
			Method m = this.getClass().getMethod("addContentAssistProcessor", IContentAssistProcessor.class,
					String.class);
			if (m != null) {
		...
		if (processor != DEFAULT_CONTENT_ASSIST_PROCESSOR) {
			...
			try {
				...
				setContentAssistProcessor(null, null);
			}
		}
	}
