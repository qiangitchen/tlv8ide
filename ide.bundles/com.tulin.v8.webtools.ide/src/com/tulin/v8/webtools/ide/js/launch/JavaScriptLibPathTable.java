...
public class JavaScriptLibPathTable {
	...
	private List<Object> tableModel = new ArrayList<Object>();
	 ...
	 */
	public JavaScriptLibPathTable(final Composite parent) {
		...
		tableViewer.setLabelProvider(new ITableLabelProvider() {
			public Image getColumnImage(Object element, int columnIndex) {
				if (element instanceof File) {
					return WebToolsPlugin.getDefault().getImageRegistry().get(WebToolsPlugin.ICON_FOLDER);
		...
		add.setText(WebToolsPlugin.getResourceString("Button.Add"));
		...
		add.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent evt) {
				...
				dialog.setTitle(WebToolsPlugin.getResourceString("JavaScriptPropertyPage.ChooseJavaScript"));
				dialog.setMessage(
						WebToolsPlugin.getResourceString("JavaScriptPropertyPage.ChooseJavaScript.Description"));
				dialog.setInput(wsroot);
				dialog.setValidator(new ISelectionStatusValidator() {
					private IStatus okStatus = new Status(Status.OK, WebToolsPlugin.getPluginId(), Status.OK, "", null);
					private IStatus ngStatus = new Status(Status.ERROR, WebToolsPlugin.getPluginId(), Status.ERROR, "",
							null);
				...
				if (dialog.open() == Dialog.OK) {
					...
					for (int i = 0; i < results.length; i++) {
						tableModel.add((IFolder) results[i]);
					}
					tableViewer.refresh();
					modelChanged();
				}
		...
		addExternal.setText(WebToolsPlugin.getResourceString("Button.AddExternalFolder"));
		addExternal.setLayoutData(createButtonGridData());
		addExternal.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent evt) {
				DirectoryDialog dialog = new DirectoryDialog(parent.getShell(), SWT.OPEN | SWT.MULTI);
				String result = dialog.open();
				if (result != null) {
					String dir = dialog.getFilterPath();
					if (dir != null) {
						tableModel.add(new File(dir));
						tableViewer.refresh();
						modelChanged();
					}
				}
		...
		remove.setText(WebToolsPlugin.getResourceString("Button.Remove"));
		...
		up.setText(WebToolsPlugin.getResourceString("Button.Up"));
		...
		down.setText(WebToolsPlugin.getResourceString("Button.Down"));
	...
	protected void updateButtons() {
		...
		up.setEnabled(
				tableViewer.getTable().getSelectionCount() == 1 && tableViewer.getTable().getSelectionIndex() > 0);
	 ...
	 * <p>
	 * Please override this method at the subclass, if you want to do any processing
	 * by the table model modifications.
	 */
