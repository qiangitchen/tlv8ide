...
class DToA {
	 ...
	 * that we could produce, which occurs when printing -5e-324 in binary. We could
	 * compute a better estimate of the size of the output string and malloc fewer
	 * bytes depending on d and base, but why bother?
	 */
	...
	static final int DTOSTR_STANDARD = 0, /* Either fixed or exponential format; round-trip */
			...
			DTOSTR_FIXED = 2, /*
								 * Round to <precision> digits after the decimal point; exponential if number is
								 * large
								 */
	...
	private static final double tens[] = { 1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13,
	 ...
	 * Bigint and e is the returned binary exponent. Return the number of
	 * significant bits in b in bits. d must be finite and nonzero.
	 */
			...
			do {
				...
				if (j1 == 0 && ((word1 & 1) == 0)) {
					...
					if (j1 > 0) {
						...
						j1 = b.compareTo(s);
						if (j1 > 0) /*
									 * The even test (|| (j1 == 0 && (digit & 1))) is not here because it messes up
									 * odd base output such as 3.5 in base 3.
									 */
							digit++;
					}
					done = true;
...
//                JS_ASSERT(digit < (uint32)base);
	 ...
	 *
	 * Modifications: 1. Rather than iterating, we use a simple numeric overestimate
	 * to determine k = floor(log10(d)). We scale relevant quantities using
	 * O(log2(k)) rather than O(k) multiplications. 2. For some modes > 2
	 * (corresponding to ecvt and fcvt), we don't try to generate digits strictly
	 * left to right. Instead, we compute with fewer bits and propagate the carry if
	 * necessary when rounding the final digit up. This is often faster. 3. Under
	 * the assumption that input will be rounded nearest, mode 0 renders 1e23 as
	 * 1e23 rather than 9.999999999999999e22. That is, we allow equality in stopping
	 * tests when the round-nearest rule will give the same floating-point value as
	 * would satisfaction of the stopping test with strict inequality. 4. We remove
	 * common factors of powers of 2 from relevant quantities. 5. When converting
	 * floating-point integers less than 1e16, we use floating-point arithmetic
	 * rather than resorting to multiple-precision integers. 6. When asked to
	 * produce fewer than 15 digits, we first try to get by with floating-point
	 * arithmetic; we resort to multiple-precision integer arithmetic only if we
	 * cannot guarantee that the floating-point calculation has given the correctly
	 * rounded result. For k requested digits and "uniformly" distributed input, the
	 * probability is something like 10^(k-15) that we must resort to the Long
	 * calculation.
	 */
	 ...
	 * when the number is exactly halfway between two representable values. For
	 * example, rounding 2.5 to zero digits after the decimal point will return 3
	 * and not 2. 2.49 will still round to 2, and 2.51 will still round to 3.
	 */
	/*
	 * bufsize should be at least 20 for modes 0 and 1. For the other modes, bufsize
	 * should be two greater than the maximum number of output characters expected.
	 */
	static int JS_dtoa(double d, int mode, boolean biasUp, int ndigits, boolean[] sign, StringBuffer buf) {
		/*
		 * Arguments ndigits, decpt, sign are similar to those of ecvt and fcvt;
		 * trailing zeros are suppressed from the returned string. If not null, *rve is
		 * set to point to the end of the return value. If d is +-Infinity or NaN, then
		 * *decpt is set to 9999.
		 * 
		 * mode: 0 ==> shortest string that yields d when read in and rounded to
		 * nearest. 1 ==> like 0, but with Steele & White stopping rule; e.g. with IEEE
		 * P754 arithmetic , mode 0 gives 1e23 whereas mode 1 gives
		 * 9.999999999999999e22. 2 ==> max(1,ndigits) significant digits. This gives a
		 * return value similar to that of ecvt, except that trailing zeros are
		 * suppressed. 3 ==> through ndigits past the decimal point. This gives a return
		 * value similar to that from fcvt, except that trailing zeros are suppressed,
		 * and ndigits can be negative. 4-9 should give the same return values as 2-3,
		 * i.e., 4 <= mode <= 9 ==> same return as mode 2 + (mode & 1). These modes are
		 * mainly for debugging; often they run slower but sometimes faster than modes
		 * 2-3. 4,5,8,9 ==> left-to-right digit generation. 6-9 ==> don't try fast
...
//          no_digits:
		...
		if ((i = (word0(d) >>> Exp_shift1 & (Exp_mask >> Exp_shift1))) != 0) {
			 ...
			 * log(1.5)/log(10) + (x-1.5)/(1.5*log(10)) log10(d) = (i-Bias)*log(2)/log(10) +
			 * log10(d2)
			 *
			 ...
			 * 0.176091259055681 );
			 *
			 * We want k to be too large rather than too small. The error in the first-order
			 * Taylor series approximation is in our favor, so we just round up the constant
			 * enough to compensate for any error in the multiplication of (i - Bias) by
			 * 0.301029995663981; since |i - Bias| <= 1077, and 1077 * 0.30103 * 2^-52 ~=~
			 * 7.2e-14, adding 1e-13 to the constant term more than suffices. Hence we
			 * adjust the constant term to 0.1760912590558. (We could get a more accurate k
			 * by invoking log10, but this is probably not worthwhile.)
			 */
			i -= Bias;
			denorm = false;
		} else {
...
//            word0(d2) -= 31*Exp_msk1; /* adjust exponent */
		...
		/*
		 * At this point floor(log10(d)) <= k <= floor(log10(d))+1. If k_check is zero,
		 * we're guaranteed that k = floor(log10(d)).
		 */
		j = bbits[0] - i - 1;
		 ...
		 * integer, b2 >= 0, b5 >= 0, s2 >= 0, and s5 >= 0.
		 */
		if (mode < 0 || mode > 9)
			mode = 0;
		try_quick = true;
		 ...
		 * ndigits, when it turns out that k was computed too high by one.
		 */
...
//                        ++*s++;
		...
		if (leftright) {
			...
			/*
			 * (mhi * 2^m2 * 5^m5) / (2^s2 * 5^s5) = one-half of last printed (when mode >=
			 * 2) or input (when mode < 2) significant digit, divided by 10^k.
			 */
		}
		/*
		 * We still have d/10^k = (b * 2^b2 * 5^b5) / (2^s2 * 5^s5). Reduce common
		 * factors in b2, m2, and s2 without changing the equalities.
		 */
		if (m2 > 0 && s2 > 0) {
			i = (m2 < s2) ? m2 : s2;
			b2 -= i;
		...
		/*
		 * Now we have d/10^k = (b * 2^b2) / (2^s2 * 5^s5) and (mhi * 2^m2) / (2^s2 *
		 * 5^s5) = one-half of last printed or input significant digit, divided by 10^k.
		 */
		...
		if (s5 > 0)
			S = pow5mult(S, s5);
		/*
		 * Now we have d/10^k = (b * 2^b2) / (S * 2^s2) and (mhi * 2^m2) / (S * 2^s2) =
		...
		/*
		 * Arrange for convenient computation of quotients: shift left if necessary so
		 * divisor has 4 leading 0 bits.
		 *
		 * Perhaps we should just compute leading 28 bits of S once and for all and pass
		 * them and a shift to quorem, so it can do shifts and ors to compute the
		 * numerator for q.
		 */
		...
		if (s2 > 0)
			S = S.shiftLeft(s2);
		/*
		 * Now we have d/10^k = b/S and (mhi * 2^m2) / S = maximum acceptable error,
		 * divided by 10^k.
		 */
		if (k_check) {
			if (b.compareTo(S) < 0) {
		...
		if (ilim <= 0 && mode > 2) {
			/*
			 * We're doing fixed-mode output and d is less than the minimum nonzero output
			 * in this mode. Output either zero or the minimum nonzero output depending on
			 * which is closer to d.
			 */
...
//        one_digit:
		...
		if (leftright) {
			...
			/*
			 * mlo/S = maximum acceptable error, divided by 10^k, if the output is less than
			 * d.
			 */
			/*
			 * mhi/S = maximum acceptable error, divided by 10^k, if the output is greater
			 * than d.
			 */
...
//                DTOSTR_VARIABLE_BUFFER_SIZE(precision)));
		...
		if (mode == DTOSTR_FIXED && (d >= 1e21 || d <= -1e21))
			mode = DTOSTR_STANDARD; /*
									 * Change mode here rather than below because the buffer may not be large enough
									 * to hold a large integer.
									 */
...
//                JS_ASSERT(decPt <= nDigits);
		...
		if (sign[0] && !(word0(d) == Sign_bit && word1(d) == 0)
