...
public class JSPDirectiveAssistProcessor implements IContentAssistProcessor {
	...
	static {
		...
		ATTRIBUTES.put("page", new String[] { "language", "extends", "import", "session", "buffer", "autoFlush",
				"isThreadSafe", "info", "errorPage", "isErrorPage", "contentType", "pageEncoding" });
	...
	private ICompletionProposal[] getAttributeValues(String directive, String attr, String lastWord, int offset) {
		...
		if (directive.equals("page")) {
			if (attr.equals("autoFlush") || attr.equals("session") || attr.equals("isThreadSafe")
					|| attr.equals("isErrorPage")) {
				values.add("true");
				values.add("false");
			}
		}
		...
		if (directive.equals("include")) {
			if (attr.equals("file")) {
				...
				for (IFileAssistProcessor processor : fileAssistProcessors) {
					processor.reload(file);
					for (AssistInfo info : processor.getAssistInfo(lastWord.substring(1))) {
						assistInfos.add(new CompletionProposal(info.getReplaceString(), offset - lastWord.length() + 1,
								lastWord.length() - 1, info.getReplaceString().length(), info.getImage(),
								info.getDisplayString(), null, null));
					}
				}
		...
		for (String value : values) {
			if (value.startsWith(lastWord)) {
				assistInfos.add(new CompletionProposal(value, offset - lastWord.length(), lastWord.length(),
						value.length(), valueImage, value, null, null));
			}
		}
	...
	private ICompletionProposal[] getDirectives(String lastWord, int offset) {
		...
		for (int i = 0; i < DIRECTIVES.length; i++) {
			...
			if (directive.startsWith(lastWord)) {
				assistInfos.add(new CompletionProposal(directive, offset - lastWord.length(), lastWord.length(),
						directive.length(), tagImage, directive, null, null));
			}
		}
	...
	private ICompletionProposal[] getAttributes(String directive, String lastWord, int offset) {
		...
		for (int i = 0; i < attrs.length; i++) {
			...
			if (attrName.startsWith(lastWord)) {
				assistInfos.add(new CompletionProposal(replace, offset - lastWord.length(), lastWord.length(),
						replace.length() - 1, attrImage, attrName, null, null));
			}
		}
