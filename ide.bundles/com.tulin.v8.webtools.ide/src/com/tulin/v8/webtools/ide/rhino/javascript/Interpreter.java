...
import com.tulin.v8.webtools.ide.rhino.javascript.ScriptRuntime.NoSuchMethodShim;
import com.tulin.v8.webtools.ide.rhino.javascript.debug.DebugFrame;
...
// Additional interpreter-specific codes
	...
	Icode_DUP = -1,
			...
			// various types of ++/--
			Icode_VAR_INC_DEC = -7, Icode_NAME_INC_DEC = -8, Icode_PROP_INC_DEC = -9, Icode_ELEM_INC_DEC = -10,
			...
			Icode_REG_IND_C0 = -32, Icode_REG_IND_C1 = -33, Icode_REG_IND_C2 = -34, Icode_REG_IND_C3 = -35,
			Icode_REG_IND_C4 = -36, Icode_REG_IND_C5 = -37, Icode_REG_IND1 = -38, Icode_REG_IND2 = -39,
			Icode_REG_IND4 = -40,

			// Load string register to prepare for the following string operation
			Icode_REG_STR_C0 = -41, Icode_REG_STR_C1 = -42, Icode_REG_STR_C2 = -43, Icode_REG_STR_C3 = -44,
			Icode_REG_STR1 = -45, Icode_REG_STR2 = -46, Icode_REG_STR4 = -47,
...
// The values that change during interpretation
	...
	public Object compile(CompilerEnvirons compilerEnv, ScriptOrFnNode tree, String encodedSource,
	...
	private void visitExpression(Node node, int contextFlags) {
		...
		case Token.NEW: {
			...
			if (callType != Node.NON_SPECIALCALL) {
				...
				if (type == Token.CALL && (contextFlags & ECF_TAIL) != 0 && !compilerEnv.isGenerateDebugInfo()
						&& !itsInTryFlag) {
	...
	private void addExceptionHandler(int icodeStart, int icodeEnd, int handlerStart, boolean isFinally,
	...
	private static int getInt(byte[] iCode, int pc) {
		return (iCode[pc] << 24) | ((iCode[pc + 1] & 0xFF) << 16) | ((iCode[pc + 2] & 0xFF) << 8)
				| (iCode[pc + 3] & 0xFF);
	}
	...
	private static void dumpICode(InterpreterData idata) {
		...
		if (table != null) {
			...
			for (int i = 0; i != table.length; i += EXCEPTION_SLOT_SIZE) {
				...
				out.println(" tryStart=" + tryStart + " tryEnd=" + tryEnd + " handlerStart=" + handlerStart + " type="
						+ (type == 0 ? "catch" : "finally") + " exceptionLocal=" + exceptionLocal);
			}
		}
		out.flush();
	}
	...
	public String getPatchedStack(RhinoException ex, String nativeStackTrace) {
		String tag = "com.tulin.v8.webtools.ide.jseditor.rhino.javascript.Interpreter.interpretLoop";
	...
	private static Object interpretLoop(Context cx, CallFrame frame, Object throwable) {
		...
		StateLoop: for (;;) {
					...
					jumplessRun: {
						...
						case Icode_GENERATOR: {
							if (!frame.frozen) {
								...
								NativeGenerator generator = new NativeGenerator(frame.scope, generatorFrame.fnOrScript,
										generatorFrame);
						...
						case Icode_GENERATOR_END: {
							...
							generatorState.returnedException = new JavaScriptException(
									NativeIterator.getStopIterationObject(frame.scope), frame.idata.itsSourceFile,
									sourceLine);
							break Loop;
						}
						case Token.THROW: {
						...
						case Icode_CALLSPECIAL: {
							...
							if (isNew) {
								...
								stack[stackTop] = ScriptRuntime.newSpecial(cx, function, outArgs, frame.scope,
										callType);
							} else {
								...
								stack[stackTop] = ScriptRuntime.callSpecial(cx, function, functionThis, outArgs,
										frame.scope, frame.thisObj, callType, frame.idata.itsSourceFile, sourceLine);
							}
							frame.pc += 4;
							continue Loop;
						}
						...
						case Token.REF_CALL: {
							...
							if (fun instanceof InterpretedFunction) {
								...
								if (frame.fnOrScript.securityDomain == ifun.securityDomain) {
									...
									initFrame(cx, calleeScope, funThisObj, stack, sDbl, stackTop + 2, indexReg, ifun,
								...
								if (BaseFunction.isApplyOrCall(ifun)) {
									...
									if (applyCallable instanceof InterpretedFunction) {
										...
										if (frame.fnOrScript.securityDomain == iApplyCallable.securityDomain) {
											frame = initFrameForApplyOrCall(cx, frame, indexReg, stack, sDbl, stackTop,
													op, calleeScope, ifun, iApplyCallable);
											continue StateLoop;
										}
									}
								}
							...
							if (fun instanceof NoSuchMethodShim) {
								...
								if (noSuchMethodMethod instanceof InterpretedFunction) {
									...
									if (frame.fnOrScript.securityDomain == ifun.securityDomain) {
										frame = initFrameForNoSuchMethod(cx, frame, indexReg, stack, sDbl, stackTop, op,
												funThisObj, calleeScope, noSuchMethodShim, ifun);
										continue StateLoop;
									}
								}
							}
						...
						case Token.NEW: {
							...
							if (lhs instanceof InterpretedFunction) {
								...
								if (frame.fnOrScript.securityDomain == f.securityDomain) {
									...
									CallFrame calleeFrame = new CallFrame();
									initFrame(cx, frame.scope, newInstance, stack, sDbl, stackTop + 1, indexReg, f,
											frame, calleeFrame);
						...
						case Token.CATCH_SCOPE: {
							...
							stack[indexReg] = ScriptRuntime.newCatchScope(caughtException, lastCatchScope, stringReg,
									cx, frame.scope);
							++frame.pc;
							continue Loop;
						}
						...
						case Token.ENUM_INIT_ARRAY: {
							...
							--stackTop;
							indexReg += frame.localShift;
							int enumType = op == Token.ENUM_INIT_KEYS ? ScriptRuntime.ENUMERATE_KEYS
									: op == Token.ENUM_INIT_VALUES ? ScriptRuntime.ENUMERATE_VALUES
						...
						case Token.ENUM_ID: {
							...
							++stackTop;
							stack[stackTop] = (op == Token.ENUM_NEXT) ? (Object) ScriptRuntime.enumNext(val)
						...
						case Icode_SCOPE_SAVE:
							...
							continue Loop;
						case Icode_CLOSURE_EXPR:
							stack[++stackTop] = InterpretedFunction.createFunction(cx, frame.scope, frame.fnOrScript,
									indexReg);
							continue Loop;
						case Icode_CLOSURE_STMT:
			...
			if (generatorState != null && generatorState.operation == NativeGenerator.GENERATOR_CLOSE
	 ...
	 */
	private static CallFrame initFrameForNoSuchMethod(Context cx, CallFrame frame, int indexReg, Object[] stack,
			double[] sDbl, int stackTop, int op, Scriptable funThisObj, Scriptable calleeScope,
	...
	private static boolean shallowEquals(Object[] stack, double[] sDbl, int stackTop) {
		Object rhs = stack[stackTop + 1];
		Object lhs = stack[stackTop];
		final Object DBL_MRK = UniqueTag.DOUBLE_MARK;
		double rdbl, ldbl;
	...
	private static CallFrame processThrowable(Context cx, Object throwable, CallFrame frame, int indexReg,
	...
	private static CallFrame initFrameForApplyOrCall(Context cx, CallFrame frame, int indexReg, Object[] stack,
			double[] sDbl, int stackTop, int op, Scriptable calleeScope, IdFunctionObject ifun,
		...
		if (BaseFunction.isApply(ifun)) {
			...
			initFrame(cx, calleeScope, applyThis, callArgs, null, 0, callArgs.length, iApplyCallable, frame,
					calleeFrame);
		} else {
			// Shift args left
			...
			initFrame(cx, calleeScope, applyThis, stack, sDbl, stackTop + 2, argCount, iApplyCallable, frame,
					calleeFrame);
		}
	...
	private static void initFrame(Context cx, Scriptable callerScope, Scriptable thisObj, Object[] args,
			double[] argsDbl, int argShift, int argCount, InterpretedFunction fnOrScript, CallFrame parentFrame,
			CallFrame frame) {
				...
				for (;;) {
					if (scope instanceof NativeWith) {
						...
						if (scope == null || (frame.parentFrame != null && frame.parentFrame.scope == scope)) {
							...
							Kit.codeBug();
							break; // Never reached, but keeps the static analyzer happy about "scope" not being
									// null 5 lines above.
						}
					} else {
						break;
	...
	private static NativeContinuation captureContinuation(Context cx, CallFrame frame,
		...
		if (requireContinuationsTopFrame) {
			...
			if (!outermost.isContinuationsTopFrame) {
				throw new IllegalStateException(
						"Cannot capture continuation " + "from JavaScript code not called directly by "
