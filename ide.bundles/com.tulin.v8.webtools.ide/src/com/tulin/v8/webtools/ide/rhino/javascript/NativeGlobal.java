...
public class NativeGlobal implements Serializable, IdFunctionCall {
	...
	public static void init(Context cx, Scriptable scope, boolean sealed) {
		...
		ScriptableObject.defineProperty(scope, "Infinity", ScriptRuntime.wrapNumber(Double.POSITIVE_INFINITY),
				ScriptableObject.DONTENUM);
		ScriptableObject.defineProperty(scope, "undefined", Undefined.instance, ScriptableObject.DONTENUM);

		String[] errorMethods = { "ConversionError", "EvalError", "RangeError", "ReferenceError", "SyntaxError",
				"TypeError", "URIError", "InternalError", "JavaException" };

		/*
		 * Each error constructor gets its own Error object as a prototype, with the
		 * 'name' property set to the name of the error.
		 */
	 ...
	 * 
	 * Includes code for the 'mask' argument supported by the C escape method, which
	 * used to be part of the browser imbedding. Blame for the strange constant
	 * names should be directed there.
	 */
	...
	private Object js_escape(Object[] args) {
		...
		for (int k = 0, L = s.length(); k != L; ++k) {
			int c = s.charAt(k);
			if (mask != 0 && ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '@'
					|| c == '*' || c == '_' || c == '-' || c == '.'
					|| (0 != (mask & URL_PATH) && (c == '/' || c == '+')))) {
				if (sb != null) {
					sb.append((char) c);
	 ...
	 */
	public static EcmaError constructError(Context cx, String error, String message, Scriptable scope,
			String sourceName, int lineNumber, int columnNumber, String lineSource) {
	...
	private static final int Id_decodeURI = 1, Id_decodeURIComponent = 2, Id_encodeURI = 3, Id_encodeURIComponent = 4,
			Id_escape = 5, Id_eval = 6, Id_isFinite = 7, Id_isNaN = 8, Id_isXMLName = 9, Id_parseFloat = 10,
			Id_parseInt = 11, Id_unescape = 12, Id_uneval = 13,
