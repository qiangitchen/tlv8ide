package com.tulin.v8.ide.wizards.directFlow;
...
public class DirectFlowPermision extends WizardPage {
	...
	public DirectFlowPermision() {
		super("directFlowPermision");
	}
	...
	private Composite setDatapermision(Composite parent) {
		...
		button3.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				if (dbkey == null || "".equals(dbkey) || table == null
						|| "".equals(table)) {
					MessageDialog
							.openError(getShell(), Messages.getString("wizards.dataselect.message.errtitle"), "请先选择dbkey和主表名称!");
				} else {
					TableSelectDialog dial = new TableSelectDialog(getShell(),
							dbkey);
					int result = dial.open();
		...
		button4.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				if (dbkey == null || "".equals(dbkey) || subtable == null
						|| "".equals(subtable)) {
					MessageDialog.openError(getShell(), Messages.getString("wizards.dataselect.message.errtitle"), Messages.getString("wizards.dataselect.message.errSelectDataSubtable"));
				} else {
					TablecellSelectDialog dial = new TablecellSelectDialog(
							getShell(), dbkey, subtable);
					int result = dial.open();
					if (IDialogConstants.OK_ID == result) {
	...
	private Composite setTablepermision(Composite shell) {
		...
		final Table tablegrid = new Table(composite, SWT.BORDER | SWT.V_SCROLL
				| SWT.FULL_SELECTION);
		...
		adddel.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (result) {
					...
					setColums(tablegrid.getItem(tablegrid.getSelectionIndex())
							.getText(), tablegrid, null, null, false);
					setPageCompleteEnable();
				}
			}
		});
						...
						if (rectangle.contains(point) && i != 3) {
							...
							Listener textListener = new Listener() {
								public void handleEvent(final Event event) {
									...
									case SWT.FocusOut:
										item.setText(column, text.getText());
										if (column == 1) {
											sublabels.put(item.getText(0),
													text.getText());
										} else if (column == 2) {
											subwidths.put(item.getText(0),
													text.getText());
										}
										text.dispose();
										break;
							...
							Listener comboListener = new Listener() {
								public void handleEvent(final Event event) {
									...
									case SWT.FocusOut:
										item.setText(3, combo.getText());
										subdatatypes.put(item.getText(0),
												combo.getText());
										combo.dispose();
										break;
									case SWT.Traverse:
				...
				if (dbkey == null || "".equals(dbkey) || subtable == null
					...
					if (IDialogConstants.OK_ID == result) {
						...
						if (cells != null && !"".equals(cells)) {
							String[] cellarray = cells.split(",");
							for (int i = 0; i < cellarray.length; i++) {
								setsubColums(cellarray[i], tablegrid, dataType,
										label, true);
							}
						}
						setPageCompleteEnable();
		...
		adddel.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (result) {
					tablegrid.remove(tablegrid.getSelectionIndex());
					setsubColums(
							tablegrid.getItem(tablegrid.getSelectionIndex())
									.getText(), tablegrid, null, null, false);
					setPageCompleteEnable();
				}
			}
	...
	private void setsubColums(String id, Table tablegrid,
			Map<String, String> datatype, Map<String, String> label,
			boolean isadd) {
		TableItem[] tbItems = tablegrid.getItems();
		int chIdIndex = -1;
	...
	private void setPageCompleteEnable() {
		boolean enable = (dbkey != null & table != null && subtable != null
				&& !"".equals(subtable) && subdirect != null
	...
	public IWizardPage getNextPage() {
		return getWizard().getPage("directFlowPageEnd");
	}
