package com.tulin.v8.ide.wizards.gridCheckDialog;
...
public class CheckListPermision extends WizardPage {
	...
	public CheckListPermision() {
		super("checkListPermision");
	}
		...
		ptable.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				if (Messages.getString("wizardsaction.dataselect.message.dataperm").equals(name)) {
					setTitle("列表多选对话框-主数据");
					stackLayout.topControl = dataComp;
				}
				if ("列表配置".equals(name)) {
					setTitle("列表多选对话框-列表配置");
					stackLayout.topControl = confComp;
				}
				if ("对话框返回信息配置".equals(name)) {
					setTitle("列表多选对话框-对话框返回信息配置");
	...
	public IWizardPage getNextPage() {
		return getWizard().getPage("checkListPageEnd");
	}
