			...
			public void run() {
				while (isRunning) {
					try {
						...
						// Too risky if there are multiple network interfaces. Need to find a better
						// way...
...
//            }
	...
	public String getResourcePathURL(String codeBase, String resourcePath) {
		...
		if (Boolean.parseBoolean(NSSystemProperty.WEBSERVER_ACTIVATEOLDRESOURCEMETHOD.get())) {
			if (Utils.IS_WINDOWS) {
				// '\' is not allowed in URL, and it is causing a problem with certain URL
				// handlers. Let's replace with '/'
		...
		if (index != -1) {
			...
			if ("location".equals(type)) {
				...
				try {
					...
					// Exception when creating a URL, so it is malformed: it is likely to be a local
					// file.
