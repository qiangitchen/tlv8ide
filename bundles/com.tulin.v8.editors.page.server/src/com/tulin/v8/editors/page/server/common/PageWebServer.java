...
//					}
	...
	public static WebServerContent getWebServerContent(Request httpRequest) {
		...
		if (index != -1) {
			...
			if ("class".equals(type)) {
				...
				PageWebServer webServer = (PageWebServer) ObjectRegistry.getInstance()
			...
			if ("classpath".equals(type)) {
				index = parameter.indexOf('/');
				final PageWebServer webServer = (PageWebServer) ObjectRegistry.getInstance()
						.get(Integer.parseInt(parameter.substring(0, index)));
				if (webServer == null) {
					return null;
				}
				...
				return new WebServerContent() {
					...
					public InputStream getInputStream() {
						try {
							...
							return PageWebServer.class.getResourceAsStream('/' + resourcePath);
						} catch (Exception e) {
							e.printStackTrace();
							return null;
						}
	...
	private static PageWebServer webServer;
	...
	static {
		...
		PageWebServer.hostAddress = hostAddress;
	}
	...
	public static PageWebServer getDefaultWebServer() {
		synchronized (LOCK) {
			if (webServer != null) {
				return webServer;
			}
			webServer = new PageWebServer();
			try {
