...
import com.tulin.v8.ureport.server.DatasourceServletAction;
import com.tulin.v8.ureport.server.DesignerServletAction;
import com.tulin.v8.ureport.server.chart.ChartServletAction;
import com.tulin.v8.ureport.server.excel.ExportExcel97ServletAction;
import com.tulin.v8.ureport.server.excel.ExportExcelServletAction;
import com.tulin.v8.ureport.server.html.HtmlPreviewServletAction;
import com.tulin.v8.ureport.server.importexcel.ImportExcelServletAction;
import com.tulin.v8.ureport.server.pdf.ExportPdfServletAction;
...
public class UReportWebServer {
	...
	public static class HTTPRequest implements Cloneable {
		...
		public String getParameter(String param) {
			...
			if (queryParameterMap.containsKey(param)) {
				res = queryParameterMap.get(param);
			} else if (httpPostDataArray != null) {
				for (int i = 0; i < httpPostDataArray.length; i++) {
					HTTPData data = httpPostDataArray[i];
					if (data.headerMap.containsKey(param)) {
						res = data.headerMap.get(param);
						break;
					}
				}
			}
		...
		public Map<String, String> getParameterMap() {
			...
			if (httpPostDataArray != null) {
				for (int i = 0; i < httpPostDataArray.length; i++) {
					HTTPData data = httpPostDataArray[i];
					if (!data.headerMap.isEmpty()) {
						for (String k : data.headerMap.keySet()) {
							map.put(k, data.headerMap.get(k));
						}
					}
				}
			}
	...
	private static class WebServerConnectionThread extends Thread {
		...
		static void writeHTTPHeaders(BufferedOutputStream out, int code, WebServerContent webscontent) {
			StringBuilder sb = new StringBuilder();
			sb.append("HTTP/1.0 " + code + " OK" + LS);
			sb.append("Content-Type: " + webscontent.getContentType() + LS);
			...
			if (webscontent.getContentLength() != -1) {
				sb.append("Content-Length: " + webscontent.getContentLength() + LS);
			}
			if (webscontent.getContentDisposition() != null) {
				sb.append("Content-Disposition: " + webscontent.getContentDisposition() + LS);
			}
			sb.append(LS);
			try {
...
//								dataContent = sb.toString();
					...
					if (httpRequest.getURLPath().startsWith("/ureport/")) {
						if (httpRequest.getURLPath().startsWith("/ureport/designer/")) {
							webServerContent = new DesignerServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/datasource/")) {
							webServerContent = new DatasourceServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/res/")) {
							webServerContent = new ResourceLoaderServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/import")) {
							webServerContent = new ImportExcelServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/preview")) {
							webServerContent = new HtmlPreviewServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/pdf")) {
							webServerContent = new ExportPdfServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/word")) {
							webServerContent = new ExportWordServletAction(httpRequest).execute();
						} else if (httpRequest.getURLPath().startsWith("/ureport/excel/")) {
							webServerContent = new ExportExcelServletAction(httpRequest).execute();
						} else if (httpRequest.getRequestURI().equals("/ureport/excel")) {
							webServerContent = new ExportExcelServletAction(httpRequest).execute();
					...
					if (isPrintRequestsDebug || isPrintDataDebug) {
						System.err.println(
								"Web Server " + (isPostMethod ? "POST" : "GET") + ": " + resourcePath + " -> 200 (OK)");
					}
					BufferedInputStream resourceStream = new BufferedInputStream(resourceStream_);
					writeHTTPHeaders(out, 200, webServerContent);
	...
	public UReportWebServer() {
		this(0);
	}

	public UReportWebServer(int port) {
		this.port = port;
	}
	...
	public String getResourcePathURL(String codeBase, String resourcePath) {
		...
		if (Boolean.parseBoolean(NSSystemProperty.WEBSERVER_ACTIVATEOLDRESOURCEMETHOD.get())) {
			if (Utils.IS_WINDOWS) {
				// '\' is not allowed in URL, and it is causing a problem with
				// certain URL handlers. Let's replace with '/'
	...
	/**
	 * Add a content provider for content that is not natively supported by the web
	 * server.
	 * 
	...
	public static WebServerContent getWebServerContent(HTTPRequest httpRequest) {
		...
		if (index != -1) {
			...
			if ("class".equals(type)) {
				...
				UReportWebServer webServer = (UReportWebServer) ObjectRegistry.getInstance()
				...
				try {
					...
					Method getWebServerContentMethod = clazz.getDeclaredMethod("getWebServerContent",
							HTTPRequest.class);
			...
			if ("classpath".equals(type)) {
				index = parameter.indexOf('/');
				final UReportWebServer webServer = (UReportWebServer) ObjectRegistry.getInstance()
						.get(Integer.parseInt(parameter.substring(0, index)));
				if (webServer == null) {
					return null;
				}
				...
				return new WebServerContent() {
					...
					public InputStream getInputStream() {
						try {
							...
							return UReportWebServer.class.getResourceAsStream('/' + resourcePath);
						} catch (Exception e) {
							e.printStackTrace();
							return null;
						}
			...
			if ("location".equals(type)) {
				...
				try {
					...
					// Exception when creating a URL, so it is malformed: it is
					// likely to be a local file.
	...
	private static UReportWebServer webServer;
	...
	static {
		...
		UReportWebServer.hostAddress = hostAddress;
	}
	...
	public static UReportWebServer getDefaultWebServer() {
		synchronized (LOCK) {
			if (webServer != null) {
				return webServer;
			}
			webServer = new UReportWebServer();
			try {
