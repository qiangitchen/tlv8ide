...
import com.tulin.v8.xml.EditorExecutionContext;
import com.tulin.v8.xml.EncodingSupport;
import com.tulin.v8.xml.FoldingActionGroup;
import com.tulin.v8.xml.internal.quickoutline.QuickOutlineHandler;
import com.tulin.v8.xml.internal.quickoutline.QuickOutlinePopupDialog;
import com.tulin.v8.xml.internal.style.SemanticHighlightingManager;

@SuppressWarnings({ "unchecked", "rawtypes", "restriction", "deprecation", "unused" })
public class XMLEditor extends TextEditor {
	...
	private class InternalModelStateListener implements IModelStateListener {
		...
		/**
		 * This 'Runnable' should be very brief, and should not "call out" to other code
		 * especially if it depends on the state of the model.
		 * 
		 * @param r
		 */
	...
	private class OutlinePageListener implements IDoubleClickListener, ISelectionChangedListener {
		...
		public void selectionChanged(SelectionChangedEvent event) {
			/*
			 * Do not allow selection from other parts to affect selection in the text
			 * widget if it has focus, or if we're still firing a change of selection.
			 * Selection events "bouncing" off of other parts are all that we can receive if
			 * we have focus (since we forwarded our selection to the service just a moment
			 * ago), and only the user should affect selection if we have focus.
			 */
	...
	private class ShowInTargetListAdapter implements IShowInTargetList {
		 ...
		 * @return the array of ID Strings that define the default show in targets for
		 *         this editor.
		 */
	 ...
	 * AbstractTextEditor to provide a StructuredTextSelection to post selection
	 * listeners. Listens to selection changes from the source viewer.
	 */
	private static class StructuredSelectionProvider implements IPostSelectionProvider, ISelectionValidator {
		/**
		 * A "hybrid" text and structured selection class containing the text selection
		 * and a list of selected model objects. The determination of which model
		 * objects matches the text selection is responsibility of the
		 * StructuredSelectionProvider which created this selection object.
		 */
		...
		/*
		 * Responsible for finding the selected objects within a text selection.
		 * Set/reset by the XMLEditor based on a per-model adapter on input.
		 */
		SelectionConvertor selectionConvertor = new SelectionConvertor();

		StructuredSelectionProvider(ISelectionProvider parentProvider, XMLEditor structuredTextEditor) {
			...
			if (fParentProvider instanceof IPostSelectionProvider) {
				((IPostSelectionProvider) fParentProvider)
		...
		public ISelection getSelection() {
			...
			if (!(selection instanceof IStructuredSelection) && selection instanceof ITextSelection) {
				...
				XMLEditor localEditor = getStructuredTextEditor();
				if (localEditor != null) {
					structuredModel = localEditor.getInternalModel();
					if (structuredModel != null) {
						if (localEditor.isBlockSelectionModeEnabled()) {
							/*
							 * Block selection handling - find the overlapping objects on each line, keeping
							 * in mind that the selected block may not overlap actual lines or columns of
							 * the document. IBlockTextSelection.getRegions() should handle that for us...
							 */
		...
		private XMLEditor getStructuredTextEditor() {
			return fEditor;
		}
		...
		public boolean isValid(ISelection selection) {
			...
			XMLEditor e = getStructuredTextEditor();
			...
			if (selection instanceof ITextSelection && !(selection instanceof IStructuredSelection)) {
				...
				if (localEditor != null) {
					...
					if (structuredModel != null) {
						int start = ((ITextSelection) selection).getOffset();
						int end = start + ((ITextSelection) selection).getLength();
						selection = new StructuredTextSelection(getDocument(), (ITextSelection) event.getSelection(),
								selectionConvertor.getElements(structuredModel, start, end));
					}
				}
				if (selection == null)
	...
	class TimeOutExpired extends TimerTask {
		public void run() {
			final byte[] result = new byte[1]; // Did the busy state end
												// successfully?
	...
	/**
	 * Representation of a character pairing that includes its priority based on its
	 * content type and how close it is to the content type of the file in the
	 * editor.
	 */
	...
	private class PartListener implements IPartListener {
		...
		public void partActivated(IWorkbenchPart part) {
			if (part.getAdapter(ITextEditor.class) == fEditor) {
				...
				/*
				 * Guard against possible tight timing between part creation and viewer
				 * configuration
				 */
	...
	private static final String REDO_ACTION_DESC_DEFAULT = SSEUIMessages.Redo_Text_Change__UI_; // $NON-NLS-1$ = "Redo
																								// Text Change."
	private static final String REDO_ACTION_TEXT = SSEUIMessages._Redo__0___Ctrl_Y_UI_; // $NON-NLS-1$ = "&Redo {0}
																						// @Ctrl+Y"
	private static final String REDO_ACTION_TEXT_DEFAULT = SSEUIMessages._Redo_Text_Change__Ctrl_Y_UI_; // $NON-NLS-1$ =
																										// "&Redo Text
																										// Change
																										// @Ctrl+Y"
	...
	private static final String UNDO_ACTION_DESC_DEFAULT = SSEUIMessages.Undo_Text_Change__UI_; // $NON-NLS-1$ = "Undo
																								// Text Change."
	private static final String UNDO_ACTION_TEXT = SSEUIMessages._Undo__0___Ctrl_Z_UI_; // $NON-NLS-1$ = "&Undo {0}
																						// @Ctrl+Z"
	...
	private String[] getDefinitions(String[] ids) {
		...
		/*
		 * Iterate through the configuration ids until one is found that has an
		 * activecontexts definition
		 */
	...
	private void computeAndSetDoubleClickAction() {
		/*
		 * Make double-clicking on the ruler toggle a breakpoint instead of toggling a
		 * bookmark. For lines where a breakpoint won't be created, create a bookmark
		 * through the contributed RulerDoubleClick action.
		 */
		setAction(ITextEditorActionConstants.RULER_DOUBLE_CLICK, new ToggleBreakpointAction(this, getVerticalRuler(),
				getAction(ITextEditorActionConstants.RULER_DOUBLE_CLICK)));
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ui.texteditor.ExtendedTextEditor#
	...
	protected void createActions() {
		...
		action = new TextOperationAction(SSEUIMessages.getResourceBundle(),
				StructuredTextEditorActionConstants.ACTION_NAME_CONTENTASSIST_CONTEXT_INFORMATION + UNDERSCORE, this,
		...
		markAsStateDependentAction(StructuredTextEditorActionConstants.ACTION_NAME_CONTENTASSIST_CONTEXT_INFORMATION,
				true);
		...
		// XMLEditor Action - add breakpoints (falling back to the
		...
		action = new OpenHyperlinkAction(resourceBundle,
				StructuredTextEditorActionConstants.ACTION_NAME_OPEN_FILE + UNDERSCORE, this, getSourceViewer());
	...
	protected ICharacterPairMatcher createCharacterPairMatcher() {
		...
		if (matcher == null) {
			matcher = new DefaultCharacterPairMatcher(
					new char[] { '(', ')', '{', '}', '[', ']', '<', '>', '"', '"', '\'', '\'' });
		}
		return matcher;
	}
	...
	protected PropertySheetConfiguration createPropertySheetConfiguration() {
		...
		String[] ids = getConfigurationPoints();
		for (int i = 0; cfg == null && i < ids.length; i++) {
			cfg = (PropertySheetConfiguration) builder
					.getConfiguration(ExtendedConfigurationBuilder.PROPERTYSHEETCONFIGURATION, ids[i]);
		}
		return cfg;
	}
	...
	private ISourceEditingTextTools createSourceEditingTextTools() {
		...
		try {
			method = tools.getClass().getMethod("setTextEditor", new Class[] { XMLEditor.class }); //$NON-NLS-1$
	...
	private ConfigurationAndTarget createSourceViewerConfiguration() {
		...
		String[] ids = getConfigurationPoints();
		for (int i = 0; cfg == null && i < ids.length; i++) {
			cfg = (StructuredTextViewerConfiguration) builder
					.getConfiguration(ExtendedConfigurationBuilder.SOURCEVIEWERCONFIGURATION, ids[i]);
	 ...
	 */
	public void dispose() {
		Logger.trace("Source Editor", "XMLEditor::dispose entry"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (fStructuredModel != null) {
			...
			/*
			 * BUG398460 - Editor is still marked dirty when relaunching editor after
		...
		/*
		 * Strictly speaking, but following null outs should not be needed, but in the
		 * event of a memory leak, they make the memory leak less severe
		 */
		fDropAdapter = null;
		fDropTarget = null;
		...
		Logger.trace("Source Editor", "XMLEditor::dispose exit"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	/**
	 * Disposes model specific editor helpers such as statusLineHelper. Basically
	 * any code repeated in update() & dispose() should be placed here.
	 */
	...
	/**
	 * <p>
	 * Attempts to get the {@link IStructuredModel} for the given
	 * {@link IEditorInput}
	 * </p>
	 * 
	 * @param input the {@link IEditorInput} to try and get the
	 *              {@link IStructuredModel} for
	 * 
	 * @return The {@link IStructuredModel} associated with the given
	 *         {@link IEditorInput} or <code>null</code> if no associated
	 *         {@link IStructuredModel} could be found.
	 */
	...
	public void editorContextMenuAboutToShow(IMenuManager menu) {
		/*
		 * To be consistent with the Java Editor, we want to remove ShiftRight and
		 * ShiftLeft from the context menu.
		 */
	 ...
	 * but expected to be gaurded there with ILock, plus, can be called directly
	 * from timer thread, so the timer's run method guards with ILock too.
	 * 
	...
	public Object getAdapter(Class required) {
		...
		if (ITextEditor.class.equals(required) || ITextEditorExtension5.class.equals(required)
				|| ITextEditorExtension4.class.equals(required) || ITextEditorExtension3.class.equals(required)
				|| ITextEditorExtension2.class.equals(required) || ITextEditorExtension.class.equals(required)) {
			result = this;
		...
		if (result == null) {
			// Logger.log(Logger.INFO_DEBUG, "XMLEditor.getAdapter returning null for " +
			// required); //$NON-NLS-1$
		}
		if (org.eclipse.wst.sse.core.internal.util.Debug.perfTestAdapterClassLoading) {
	...
	private String[] getConfigurationPoints() {
		...
		return ConfigurationPointCalculator.getConfigurationPoints(this, contentTypeIdentifierID,
				ConfigurationPointCalculator.SOURCE, XMLEditor.class);
	}

	/**
	 * added checks to overcome bug such that if we are shutting down in an error
	 * condition, then viewer will have already been disposed.
	 */
	 ...
	 *             instance and will be removed in the future. It is recommended
	 *             that the current document provider be asked for the current
	 *             document and the IModelManager then asked for the corresponding
	 *             model with getExistingModelFor*(IDocument). Supported document
	 *             providers ensure that the document maps to a shared structured
	 *             model.
	 */
	public IStructuredModel getModel() {
		...
		if (fStructuredModel == null && documentProvider != null) {
			...
			if (documentProvider instanceof IModelProvider) {
				...
				if (doc instanceof IStructuredDocument) {
					/*
					 * Called in this manner because getExistingModel can skip some calculations
					 * always performed in getModelForEdit
					 */
					...
					// 默认为xml
					fStructuredModel = StructuredModelManager.getModelManager()
							.createUnManagedStructuredModelFor(ContentTypeIdForXML.ContentTypeID_XML);
				}
			}
			...
			if (initialModelNull && fStructuredModel != null) {
				/*
				 * DMW: 9/1/2002 -- why is update called here? No change has been indicated? I'd
				 * like to remove, but will leave for now to avoid breaking this hack. Should
				 * measure/breakpoint to see how large the problem is. May cause performance
				 * problems.
				 * 
				 * DMW: 9/8/2002 -- not sure why this was here initially, but the intent/hack
				 * must have been to call update if this was the first time fStructuredModel was
				 * set. So, I added the logic to check for that "first time" case. It would
				 * appear we don't really need. may remove in future when can test more.
				 */
				update();
			}
		}
	...
	public ISelectionProvider getSelectionProvider() {
		if (fStructuredSelectionProvider == null) {
			...
			if (parentProvider != null) {
				...
				if (fStructuredModel != null) {
					SelectionConvertor convertor = (SelectionConvertor) fStructuredModel
							.getAdapter(SelectionConvertor.class);
					if (convertor != null) {
						fStructuredSelectionProvider.selectionConvertor = convertor;
					}
				}
	...
	/**
	 * Returns the editor's source viewer. This method was created to expose the
	 * protected final getSourceViewer() method.
	 * <p>
	 * Not API. May be removed in the future.
	 * </p>
	 * 
	...
	protected void handleElementContentReplaced() {
		...
		SourceViewerConfiguration sourceViewerConfiguration = getSourceViewerConfiguration();
		if (document != null && sourceViewerConfiguration != null
				&& sourceViewerConfiguration.getReconciler(getSourceViewer()) instanceof DirtyRegionProcessor) {
			((DirtyRegionProcessor) sourceViewerConfiguration.getReconciler(getSourceViewer()))
					.processDirtyRegion(new DirtyRegion(0, document.getLength(), DirtyRegion.INSERT, document.get()));
		}

		/*
		 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=129906 - update selection to
		 * listeners
		 */
		...
		if (selectionProvider instanceof StructuredSelectionProvider && originalSelection instanceof ITextSelection) {
			...
			if (!textSelection.isEmpty() && (document == null
					|| textSelection.getOffset() + textSelection.getLength() <= document.getLength())) {
	...
	protected void handlePreferenceStoreChanged(PropertyChangeEvent event) {
		...
		else if (EVALUATE_TEMPORARY_PROBLEMS.equals(property)) {
		...
		else if (FoldingActionGroup.FOLDING_ENABLED.equals(property)) {
		...
		else if (CODEASSIST_PROPOSALS_BACKGROUND.equals(property)) {
			ISourceViewer sourceViewer = getSourceViewer();
			if (sourceViewer != null) {
		...
		// update content assist preferences
		else if (CODEASSIST_PROPOSALS_FOREGROUND.equals(property)) {
			ISourceViewer sourceViewer = getSourceViewer();
			if (sourceViewer != null) {
				SourceViewerConfiguration configuration = getSourceViewerConfiguration();
				if (configuration != null) {
		...
		// update content assist preferences
		else if (CODEASSIST_PARAMETERS_BACKGROUND.equals(property)) {
			ISourceViewer sourceViewer = getSourceViewer();
			if (sourceViewer != null) {
				SourceViewerConfiguration configuration = getSourceViewerConfiguration();
				if (configuration != null) {
		...
		// update content assist preferences
		else if (CODEASSIST_PARAMETERS_FOREGROUND.equals(property)) {
			ISourceViewer sourceViewer = getSourceViewer();
			if (sourceViewer != null) {
				SourceViewerConfiguration configuration = getSourceViewerConfiguration();
				if (configuration != null) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.ui.texteditor.AbstractTextEditor#initializeDragAndDrop(org
	...
	private void initializeSourceViewer() {
		...
		if (openHyperlinkAction instanceof OpenHyperlinkAction) {
			((OpenHyperlinkAction) openHyperlinkAction)
					.setHyperlinkDetectors(getSourceViewerConfiguration().getHyperlinkDetectors(getSourceViewer()));
		}
	...
	private boolean isFoldingEnabled() {
		...
		return (store.getBoolean(FoldingActionGroup.FOLDING_ENABLED));
	}
	...
	private boolean isValidationEnabled() {
		return getPreferenceStore().getBoolean(EVALUATE_TEMPORARY_PROBLEMS);
	}

	/**
	 * <p>
	 * Logs a warning about how this {@link XMLEditor} just opened an
	 * {@link IEditorInput} it was not designed to open.
	 * </p>
	 * 
	 * @param input the {@link IEditorInput} this {@link XMLEditor} was not designed
	 *              to open to log the message about.
	 */
	private void logUnexpectedDocumentKind(IEditorInput input) {
		Logger.log(Logger.WARNING, "XMLEditor being used without StructuredDocument"); //$NON-NLS-1$
		String name = null;
		if (input != null) {
			name = input.getName();
		} else {
	...
	public void rememberSelection() {
		/*
		 * This method was made public for use by editors that use XMLEditor (like some
		 * clients)
		 */
	...
	protected void rulerContextMenuAboutToShow(IMenuManager menu) {
		...
		if (internalModel != null) {
			boolean debuggingAvailable = BreakpointProviderBuilder.getInstance().isAvailable(
					internalModel.getContentTypeIdentifier(), BreakpointRulerAction.getFileExtension(getEditorInput()));
	...
	/**
	 * Sets the editor's source viewer configuration which it uses to configure it's
	 * internal source viewer. This method was overwritten so that viewer
	 * configuration could be set after editor part was created.
	 */
	protected void setSourceViewerConfiguration(SourceViewerConfiguration config) {
		...
		if (stv != null) {
			/*
			 * There should be no need to unconfigure before configure because configure
			 * will also unconfigure before configuring
			 */
	...
	private void addReconcilingListeners(SourceViewerConfiguration config, StructuredTextViewer stv) {
		try {
			...
			String[] ids = getConfigurationPoints();
			for (int i = 0; i < ids.length; i++) {
				reconcilingListeners.addAll(ExtendedConfigurationBuilder.getInstance()
						.getConfigurations("sourceReconcilingListener", ids[i])); //$NON-NLS-1$
			}
			fReconcilingListeners = (ISourceReconcilingListener[]) reconcilingListeners
					.toArray(new ISourceReconcilingListener[reconcilingListeners.size()]);
	...
	public void update() {
		if (fOutlinePage != null && fOutlinePage instanceof ConfigurableContentOutlinePage) {
			...
			((ConfigurableContentOutlinePage) fOutlinePage)
					.setInputContentTypeIdentifier(internalModel.getContentTypeIdentifier());
		...
		if (getSourceViewerConfiguration() instanceof StructuredTextViewerConfiguration) {
			fStatusLineLabelProvider = ((StructuredTextViewerConfiguration) getSourceViewerConfiguration())
					.getStatusLineLabelProvider(getSourceViewer());
	...
	protected void updateContentDependentActions() {
		...
		// XMLEditor's undo and redo actions
	...
	/**
	 * Updates the editor context menu by creating a new context menu with the given
	 * menu id
	 * 
		...
		if (!contextMenuId.equals(getEditorContextMenuId()) || (fTextContextMenu == null)) {
			...
			if (getSourceViewer() != null) {
				...
				if (styledText != null) {
					...
					if (partId != null) {
						getSite().registerContextMenu(partId + EDITOR_CONTEXT_MENU_SUFFIX, fTextContextMenuManager,
								getSelectionProvider());
					}
	...
	/**
	 * Updates editor context menu, vertical ruler menu, help context id for new
	 * content type
	 * 
	 * @param contentType
	 */
	...
	private void updateEncodingMemento() {
		...
		if (internalModel != null) {
			...
			/**
			 * Be sure to use the new value but only if no exception occurred. (we may find
			 * cases we need to do more error recovery there) should be near impossible to
			 * get IOException from processing the _document_
			 */
			if (!failed) {
	...
	private void updateRulerContextMenuId(String rulerMenuId) {
		...
		if (!rulerMenuId.equals(getRulerContextMenuId()) || (fRulerContextMenu == null)) {
			...
			if (getVerticalRuler() != null) {
				...
				String partId = getSite().getId();
				if (partId != null) {
					getSite().registerContextMenu(partId + RULER_CONTEXT_MENU_SUFFIX, fRulerContextMenuManager,
							getSelectionProvider());
				}
	...
	protected SourceViewerDecorationSupport getSourceViewerDecorationSupport(ISourceViewer viewer) {
		/*
		 * Removed workaround for Bug [206913] source annotations are not painting in
		 * source editors. With the new presentation reconciler, we no longer need to
		 * force the painting. This actually caused Bug [219776] Wrong annotation
		 * display on macs. We forced the Squiggles strategy, even when the native
		 * problem underline was specified for annotations
		 */
	...
	private void installCharacterPairing() {
		...
		if (model != null) {
			IConfigurationElement[] elements = Platform.getExtensionRegistry()
					.getConfigurationElementsFor(SSEUIPlugin.ID, "characterPairing"); //$NON-NLS-1$
						...
						for (int j = 0; j < contentTypes.length; j++) {
							...
							if (id != null) {
								...
								if (priority >= 0) {
									...
									String[] partitions = StringUtils
											.unpack(contentTypes[j].getAttribute("partitions"));
									pairing.partitions = new HashSet(partitions.length);
									// Only add the inserter if there is at
									// least one partition for the content type
									for (int k = 0; k < partitions.length; k++) {
										pairing.partitions.add(partitions[k]);
									}

									pairing.inserter = (AbstractCharacterPairInserter) element
	...
	/**
	 * Calculates the priority of the target content type. The closer
	 * <code>targetType</code> is to <code>type</code> the higher its priority.
	 * 
	 * @param type
	 * @param targetType
	 * @param priority
	...
	private void installSemanticHighlighting() {
		...
		if (fSemanticManager == null && model != null) {
			...
			fSemanticManager.install(getSourceViewer(), getPreferenceStore(), getSourceViewerConfiguration(),
	...
	private IInformationPresenter configureOutlinePresenter(ISourceViewer sourceViewer,
		...
		String[] ids = getConfigurationPoints();
		for (int i = 0; cfg == null && i < ids.length; i++) {
			cfg = (AbstractQuickOutlineConfiguration) builder
					.getConfiguration(ExtendedConfigurationBuilder.QUICKOUTLINECONFIGURATION, ids[i]);
		}
