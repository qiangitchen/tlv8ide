...
/**
 * Semantic highlighting manager. Responsible for maintaining the semantic
 * highlightings and the associated styles. Semantic highlighting preference
 * changes are handled through listeners in this class. Based on
 * org.eclipse.jdt.internal.ui.javaeditor.SemanticHighlightingManager
 * 
 * @since 3.1
 */
...
public class SemanticHighlightingManager implements IPropertyChangeListener {
	...
	static class HighlightedPosition extends Position {
		...
		public HighlightedPosition(int offset, int length,
				HighlightingStyle highlighting, Object lock, boolean isReadOnly) {
			super(offset, length);
			fStyle = highlighting;
			fLock = lock;
		...
		public HighlightedPosition(Position position,
				HighlightingStyle highlighting, Object lock) {
		...
		public boolean isEqual(Position pos, HighlightingStyle highlighting) {
			synchronized (fLock) {
				return !isDeleted() && getOffset() == pos.getOffset()
	...
	/**
	 * The semantic highlighting styles associated with the semantic
	 * highlightings
	 */
	private HighlightingStyle[] fHighlightingStyles;
	/**
	 * The semantic highlighting style string preference keys. null of the
	 * highlighting doesn't support it
	 */
	...
	public void install(ISourceViewer sourceViewer,
		...
		fPresentationReconciler = (StructuredPresentationReconciler) fConfiguration
	...
	private void loadSemanticHighlightings() {
		...
		IConfigurationElement[] elements = Platform.getExtensionRegistry()
				.getConfigurationElementsFor(SSEUIPlugin.ID,
						SEMANTIC_HIGHLIGHTING_EXTENSION_POINT);

		IContentType contentType = Platform.getContentTypeManager()
				.getContentType(fContentTypeId);

		for (int i = 0; i < elements.length; i++) {
			String[] targets = StringUtils.unpack(elements[i]
					.getAttribute(TARGET_ATTR));
			for (int j = 0; j < targets.length; j++) {
				IContentType targetContentType = Platform
						.getContentTypeManager().getContentType(targets[j]);
				/* Apply semantic highlighting to kinds of targetContentType */
				if (contentType.isKindOf(targetContentType)) {
					try {
						highlighting = (ISemanticHighlighting) elements[i]
					...
					if (highlighting != null) {
						// The highlighting was not inserted through priority;
						// insert at the beginning of the list
						semantics.add(new SemanticContent(targetContentType,
								highlighting, styleKey, id, after));
					}
		...
		/*
		 * Sort the semantics, so that styles will be applied from general to
		 * specific
		 */
	...
	/**
	 * This class is used for relating a semantic highlighting to a content
	 * type. The content type is used in the sorting algorithm. Content types
	 * that are more specific to the file's content type will cause styles to be
	 * applied from general to specific
	 */
	private class SemanticContent implements Comparable {
		...
		public SemanticContent(IContentType type,
				ISemanticHighlighting highlighting, String styleKey, String id,
				String after) {
			this.type = type;
		...
		public int compareTo(Object arg0) {
			...
			if (this.type.equals(other.type)) {
				if (this.after != null && other.id != null
						&& other.id.equals(this.after)) {
					return 1;
				}
				return 0;
			}
	...
	private HighlightingStyle createHighlightingStyle(
			ISemanticHighlighting highlighting, String styleKey) {
		...
		if (store != null) {
			...
			if (styleKey != null) {
				...
				int style = getBoolean(store,
						highlighting.getBoldPreferenceKey()) ? SWT.BOLD
						: SWT.NORMAL;

				if (getBoolean(store, highlighting.getItalicPreferenceKey()))
					style |= SWT.ITALIC;
				if (getBoolean(store,
						highlighting.getStrikethroughPreferenceKey()))
				...
				if (rgbString != null)
					color = EditorUtility
							.getColor(ColorHelper.toRGB(rgbString));
				if (highlighting instanceof ISemanticHighlightingExtension2) {
					...
					if (rgbString != null) {
						bgColor = EditorUtility.getColor(ColorHelper
								.toRGB(rgbString));
					}
				}
	 ...
	 * @return text attribute created from the <code>styleValue</code> or null
	 *         if the <code>styleValue</code> is invalid
	 */
	...
	private void enable() {
		...
		fReconciler.install(fSourceViewer, fPresenter, fHighlightings,
				fHighlightingStyles);
		...
		if (reconciler instanceof DocumentRegionProcessor)
			((DocumentRegionProcessor) reconciler)
					.setSemanticHighlightingStrategy(fReconciler);
		// } else {
		// fPresenter.updatePresentation(null, createHardcodedPositions(), new
	...
	private void handleHighlightingPropertyChange(PropertyChangeEvent event) {
		...
		for (int i = 0; i < fHighlightings.length; i++) {
			...
			if (property.equals(highlighting.getItalicPreferenceKey())) {
				adaptToTextStyleChange(fHighlightingStyles[i], event,
						SWT.ITALIC);
				fPresenter.highlightingStyleChanged(fHighlightingStyles[i]);
				refreshRequired = true;
				continue;
	...
	/**
	 * Handles property changes for enabling and disabling semantic highlighting
	 * for Structured Source Editors
	 * 
	 * @param event
	 */
	 ...
	 * 
	 * @param highlighting
	 *            the highlighting style to update
	 * @param event
	 *            the event that triggered the change
	 */
	private void adaptToStyleChange(HighlightingStyle highlighting,
			PropertyChangeEvent event) {
		Object value = event.getNewValue();
	...
	private boolean isEnabled() {
		return (fPreferenceStore != null) ? fPreferenceStore
				.getBoolean(SEMANTIC_HIGHLIGHTING)
				: false;
	}
