...
import com.tulin.v8.flowdesigner.ui.CheckButtonSelectionAdapter;
import com.tulin.v8.flowdesigner.ui.OrgTreeComposite;
import com.tulin.v8.flowdesigner.ui.OrgTreeSelectionAdapter;
...
public class PersonsSelectionDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		selectallitem.setText(Messages.getString("TLEditor.PersonsSelect.checkall"));
		ToolItem cancelitem = new ToolItem(toolbar, SWT.PUSH | SWT.BORDER);
		cancelitem.setText(Messages.getString("TLEditor.PersonsSelect.uncheckall"));
		...
		psmtable = new Table(area, SWT.BORDER | SWT.V_SCROLL | SWT.FULL_SELECTION | SWT.CHECK);
		...
		containerrbot.setText(Messages.getString("TLEditor.PersonsSelect.slist"));
		containerrbot.setLayout(new GridLayout(1, false));
		ToolBar rtoolbar = new ToolBar(containerrbot, SWT.FLAT | SWT.WRAP | SWT.RIGHT);
		...
		removeitem.setImage(TuLinPlugin.getIcon("templete/reset.gif"));
		removeitem.setToolTipText(Messages.getString("TLEditor.PersonsSelect.delsel"));
		ToolItem rmvallitem = new ToolItem(rtoolbar, SWT.PUSH | SWT.BORDER);
		rmvallitem.setImage(TuLinPlugin.getIcon("templete/reset_all.gif"));
		rmvallitem.setToolTipText(Messages.getString("TLEditor.PersonsSelect.delall"));
		...
		ScrolledComposite scrolledComposite = new ScrolledComposite(containerrbot,
		...
		selectallitem.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					selectedItem.put((String) item.getData("id"), (Map) item.getData("map"));
				}
		...
		psmtable.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selected.getChecked()) {
					selectedItem.put((String) selected.getData("id"), (Map) selected.getData("map"));
				} else {
	...
	private void refreshSelecetedArea() {
			...
			check.addSelectionListener(new CheckButtonSelectionAdapter(check) {
				public void widgetSelected(SelectionEvent e, Button btn) {
					if (btn.getSelection()) {
						...
						selecetedAreaCh.put((String) btn.getData("id"), (Group) btn.getData("group"));
					} else {
		...
		selecetedArea.setSize(selecetedArea.computeSize(SWT.DEFAULT, SWT.DEFAULT));
	}
	...
	protected void initializeBounds() {
		...
		super.createButton((Composite) getButtonBar(), IDialogConstants.CANCEL_ID,
